...
unless ($blast_type eq "EST" || $blast_type eq "mRNA" || $blast_type eq "enst" || $blast_type eq "probe" || $blast_type eq "genomic" || $blast_type eq "linker"){
...
while (<PROBES>){
  ...
  if ($header_line == 1){
    $header_line = 0;
    print OUTFILE "Probe_Count\t$non_target_hits_col\t$non_target_hits_length_col\n";
    next();
  }
  ...
  unless ($probe_hits{$probe_id}){
    print OUTFILE "$probe_id\t0\t0\n";
    next();
  }

  #If there was at least one hit, report the hits as non-target hits (since a negative control probe has no target)
  print OUTFILE "$probe_id\t$probe_hits{$probe_id}{hit_count}\t$probe_hits{$probe_id}{max_align_length}\n";
}
