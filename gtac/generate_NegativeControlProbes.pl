...
GetOptions('target_length=i'=>\$target_length, 'max_length_variance=i'=>\$max_length_variance,
	   'target_tm=f'=>\$target_tm, 'tm_range=f'=>\$tm_range, 'max_cycles=i'=>\$max_cycles,
	   'bin_size=i'=>\$bin_size, 'verbose=s'=>\$verbose, 'force=s'=>\$force, 'probe_dir=s'=>\$probe_dir,
...
unless ($target_length && ($max_length_variance || $max_length_variance == 0) && $target_tm && $tm_range  && ($max_cycles =~ /\d+/) && $bin_size && $probe_dir && $outfile && $logfile){
...
print OUTFILE "Probe_Count\tProbeSet_ID\tGene_ID\tSequence\tProbe_length\tProbe_Tm\tProbe_Type\tExon1_IDs\tUnit1_start\tUnit1_end\tExon2_IDs\tUnit2_start\tUnit2_end\tmasked_bases\tNimbleGenCycles\n";
...
sub generateRandomProbe{
  ...
  #Make sure this probe does not exceed the nimble synthesis cycles limit
...
sub printProbeInfo{
  ...
  foreach my $bin (sort {$a <=> $b} keys %{$bins_ref}){
    ...
    foreach my $probe_seq (keys %{$probes_ref}){
      ...
      print OUTFILE "$current_probe_id\t$current_probeset_id\tna\t$probe_seq\t$length\t$tm_formatted\tControl-Negative\tna\tna\tna\tna\tna\tna\tna\t$cycles\n";
    }
  }
  return();
}
