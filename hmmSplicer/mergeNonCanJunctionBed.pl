#!/usr/bin/perl -w
# This script will merge a series of bed files from hmmSplicer into a single BED file - FOR THE NON-CANONICAL JUNCTION RESULTS
...
GetOptions ('results_dir=s'=>\$results_dir, 'dir_depth=s'=>\$dir_depth, 'summary_name=s'=>\$summary_name, 'track_name=s'=>\$track_name, 'ref_genome=s'=>\$ref_genome, 'min_read_count=i'=>\$min_read_count);

if ($results_dir && $dir_depth && $summary_name && $track_name && $ref_genome && $min_read_count){
  #Parameters ok
}else{
  ...
  print GREEN, "\nSpecify the minimum read count for each non-canonical junction using: --min_read_count (1 for no filtering)", RESET;
  print GREEN, "\n\nExample: mergeNonCanJunctionBed.pl  --results_dir=/projects/alexa2/hmmSplicer/SA_TN_Breast/results/HS0713/  --dir_depth=1  --summary_name=HS0713  --track_name=SA014  --ref_genome=/projects/malachig/sequence_databases/hg18_genome/all_human.fa  --min_read_count=2\n\n", RESET;
  exit();
}
#Check parameters
...
my $search;
if ($dir_depth == 1){
  $search = "ls $results_dir"."*/junction.nonCanonical.bed*";
}elsif($dir_depth == 2){
  $search = "ls $results_dir"."*/*junction.nonCanonical.bed*";
}elsif($dir_depth == 3){
  $search = "ls $results_dir"."*/results/*junction.nonCanonical.bed*";
}
...
#Remove junctions with a read count less than min_read_count reads in the library or group of libraries
print BLUE, "\n\nDeleting junctions with only 1 read from the list", RESET;
my $dc = 0;
my $t = 0;
while (my ($j) = each %junctions){
  my $rc = $junctions{$j}{read_count};
  $t++;
  unless ($rc >= $min_read_count){
    delete($junctions{$j});
    $dc++;
  }
}
print BLUE, "\n\tDeleted $dc junctions from the list of $t junctions", RESET;

#Determine dinucleotide non-canonical splice site sequence and the sequence of the whole junction by comparison back to the reference genome...
print BLUE, "\n\nObtaining splice site and junction sequence each junction", RESET;
...
while (<REF>){
  ...
  foreach my $j (sort keys %junctions){
    if ($j =~ /(.*)\:(\d+)\-(\d+)/){
      ...
      my $splice_site = "$left_dn-$right_dn";
      $junctions{$j}{strand} = ".";
      $junctions{$j}{splice_site} = "$splice_site";
...
#e.g.  track name='$track_name' description='HMMSplicer Non Canonical Junctions' useScore=1

#At the same time, write a junction ID file in simple tabular format with the read counts and scores only
my $outfile1 = "$results_dir"."$summary_name".".junction.nonCanonical.bed";
my $outfile2 = "$results_dir"."$summary_name".".junction.nonCanonical.list.txt";
...
my $track_name2 = "$track_name"."_nonCan";
print OUT1 "track name='$track_name' description='HMMSplicer Non-Canonical Junctions for $track_name ($summary_name)' useScore=1 visibility=3\n";
...
my $gzip_cmd1 = "gzip -f $outfile1";
print BLUE, "\n\nCompressing output bed file:\n$gzip_cmd1", RESET;
system($gzip_cmd1);

#Sort the junction list list
my $outfile3 = "$results_dir"."$summary_name".".junction.nc.list.tmp";
my $sort_cmd = "sort $outfile2 > $outfile3";
my $mv_cmd = "mv -f $outfile3 $outfile2";
my $gzip_cmd2 = "gzip -f $outfile2";
print BLUE, "\n\nSorting, overwritting and compressing the junction list file", RESET;
print BLUE, "\n$sort_cmd", RESET;
system($sort_cmd);
print BLUE, "\n$mv_cmd", RESET;
system($mv_cmd);
