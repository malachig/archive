...
GetOptions ('infile=s'=>\$infile, 'target_length=i'=>\$target_length,
	    ...
	    'genomic_hit_percent_limit=f'=>\$genomic_hit_percent_limit, 'nimblegen_cycles_limit=i'=>\$nimblegen_cycles_limit,
...
print GREEN, "\n\nExample filterProbes.pl  --infile=RegionProbes.txt  --target_length=54  --target_tm=76.0  --tm_range=5  --masked_percent_limit=15.0  --simfold_score_limit=-10.0  --pairfold_score_limit=-22.0  --complexity_region_percent_limit=10.0  --genomic_50percent_hits_limit=5  --genomic_75percent_hits_limit=2  --genomic_hit_percent_limit=80.0  --nimblegen_cycles_limit=180  --probeset_size=8  --outfile=RegionProbes_filtered.txt  --logfile=filter_RegionProbes_LOG.txt  --verbose=no\n\n", RESET;

unless ($infile && $target_length && $target_tm && $tm_range && $masked_percent_limit && $simfold_score_limit && $pairfold_score_limit && $complexity_region_percent_limit && $genomic_50percent_hits_limit && $genomic_75percent_hits_limit && $genomic_hit_percent_limit && $nimblegen_cycles_limit && $probeset_size && $outfile && $logfile && $verbose){
...
#TO CREATE A FILTERED SET FOR ONLY A SUBSET OF 'SPECIAL' REGION CASES:
my %sub_list;
if ($region_list){

  print YELLOW, "\nProcessing a subset of target regions only!!\n\n", RESET;

  open (RLIST, "$region_list") || die "\nCould not open region list file: $region_list\n\n";
...
sub loadProbeInfo_file{
  ...
  while (<PROBES>){
    ...
    my $region_id = $probe_line[$columns{Region_ID}{column_pos}];

    #IF specified by the user, limit import to only those probes of the regions defined in an input list
    if ($region_list){
      unless ($sub_list{$region_id}){
	next();
      }
    }
    ...
    if ($columns{'CycleCount'}){
      my $cycle_count = $probe_line[$columns{'CycleCount'}{column_pos}];
      if ($cycle_count =~ /\d+/){
...
sub printOutputFile{
  ...
  while(<INFILE>){
    ...
    if ($line[0] =~ /^(\d+)/){
      ...
      my $region_id = $line[$columns{Region_ID}{column_pos}];

      #IF specified by the user, limit print out to only those probes of the regions defined in an input list
      if ($region_list){
	unless ($sub_list{$region_id}){
	  next();
	}
