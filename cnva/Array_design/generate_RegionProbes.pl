...
# Import chromosomal regions of interest from and input file - organize into blocks according to their chromosome
# For each chromosomal region:
#     - Get the chromosomal sequence from a fasta file (both unmasked and masked) - Directory containing these files must be specified
...
GetOptions ('region_file=s'=>\$region_file, 'sequence_dir=s'=>\$sequence_dir, 'masked_bases_limit=s'=>\$masked_bases_limit,
	    'overlap=i'=>\$overlap, 'target_tm=f'=>\$target_tm, 'tm_range=f'=>\$tm_range, 'target_length=i'=>\$target_length,
	    'max_length_variance=i'=>\$max_length_variance, 'verbose=s'=>\$verbose, 'probe_dir=s'=>\$probe_dir, 'outfile=s'=>\$outfile,
	    'logfile=s'=>\$logfile);
...
print GREEN, "\n\nExample: generate_RegionProbes.pl  --region_file=MasterRegionList.txt  --sequence_dir=/home/malachig/MR_Chip_Design/hg18_genome_fasta  --masked_bases_limit=25.0  --overlap=5  --target_tm=67.0  --tm_range=3.5  --target_length=36  --max_length_variance=10  --probe_dir=/home/malachig/MR_Chip_Design/unfiltered_probes  --outfile=/home/malachig/MR_Chip_Design/unfiltered_probes/regionProbes.txt  --logfile=/home/malachig/MR_Chip_Design/logs/generate_RegionProbes_LOG.txt\n\n", RESET;

#Make sure all options were specified
unless ($region_file && $sequence_dir && $masked_bases_limit && $overlap && $target_tm && $tm_range && $target_length && ($max_length_variance || $max_length_variance eq "0") && $probe_dir && $outfile && $logfile){
...
#Check the specified sequence directory and perform sanity checks
#Make sure each sequence (chromosome) specified in the target region file has a corresponding masked and unmasked sequence file
#Also make sure that the length of masked and unmasked versions of each sequence match
chomp($sequence_dir);
unless ($sequence_dir =~ /\/$/){
  $sequence_dir = "$sequence_dir"."/";
}
...
&checkSequenceFiles('-sequence_dir'=>$sequence_dir, '-sequence_names'=>\%sequence_names);
...
print OUTFILE "Probe_Count\tProbeSet_ID\tRegion_ID\tSequence\tProbe_length\tProbe_Tm\tChromosome\tUnit1_start\tUnit1_end\tmasked_bases\n";
...
foreach my $chromosome (sort keys %sequence_names){
  ...
  #Get a sequence object for this sequence (chromosome)
  my $unmasked_file = $sequence_names{$chromosome}{unmasked_file};
  my $masked_file = $sequence_names{$chromosome}{masked_file};

  my %sequences;

  my $unmasked_seq_ref = &parseFastaFile('-file'=>$unmasked_file, '-sequence_name'=>$chromosome);
  my $masked_seq_ref = &parseFastaFile('-file'=>$masked_file, '-sequence_name'=>$chromosome);

  $sequences{$chromosome}{unmasked} = $unmasked_seq_ref;
  $sequences{$chromosome}{masked} = $masked_seq_ref;
  ...
  foreach my $region_id (@{$chr_regions_ref}){
    ...
    print BLUE, "\n\t$total_regions (REGION: $region_id)\tSize: $region_size\t... Extracting probe sequences", RESET;
    print LOG "\n\t$total_regions (REGION: $region_id)\tSize: $region_size\t... Extracting probe sequences";

    my $probes_ref;

    $probes_ref = &getRawProbes('-region_id'=>$region_id, '-chromosome'=>$chromosome, '-region_object'=>\%regions, '-sequence_object'=>\%sequences);
    my $probes_found = 0;
...
######################################################################################################################
sub checkSequenceFiles{
  my %args = @_;
  my $sequence_dir = $args{'-sequence_dir'};
  my $sequence_names_ref = $args{'-sequence_names'};

  print YELLOW, "\nAttempting to match each sequence name (chromosome) specified in the region target file to a masked/unmasked fasta file\n", RESET;
  print LOG "\nAttempting to match each sequence name (chromosome) specified in the region target file to a masked/unmasked fasta file\n";
  ...
  foreach my $sequence_name (sort keys %{$sequence_names_ref}){
    print BLUE, "\n\tChecking fasta files for $sequence_name", RESET;
    print LOG "\n\tChecking fasta files for $sequence_name";

    my $unmasked_file = "$sequence_dir"."$sequence_name".".fa";
    my $masked_file = "$sequence_dir"."$sequence_name".".fa.masked";

    unless (-e $unmasked_file){
      print RED, "\nCould not find the unmasked file: $unmasked_file for the sequence: $sequence_name!\n\n", RESET;
      exit();
    }
    unless (-e $masked_file){
      print RED, "\nCould not find the masked file: $masked_file for the sequence: $sequence_name!\n\n", RESET;
      exit();
    }

    $sequence_names_ref->{$sequence_name}->{unmasked_file} = $unmasked_file;
    $sequence_names_ref->{$sequence_name}->{masked_file} = $masked_file;
    ...
    my $masked_seq_ref = &parseFastaFile('-file'=>$masked_file, '-sequence_name'=>$sequence_name);

    my $unmasked_seqs_found = keys %{$unmasked_seq_ref};
    my $masked_seqs_found = keys %{$masked_seq_ref};

    if ($unmasked_seqs_found > 1){
      print RED, "\nFound more than one sequence in the file: $unmasked_file - Only one chromosome expected per file!\n\n", RESET;
      exit();
    }
    if ($masked_seqs_found > 1){
      print RED, "\nFound more than one sequence in the file: $masked_file - Only one chromosome expected per file!\n\n", RESET;
      exit();
    }

    #Compare length of masked and unmasked sequences
    my $unmasked_length = length(${$unmasked_seq_ref->{$sequence_name}->{sequence}});
    my $masked_length = length(${$masked_seq_ref->{$sequence_name}->{sequence}});
    unless ($unmasked_length == $masked_length){
      print RED, "\nLength of unmasked ($unmasked_length) and masked ($masked_length) sequences does not match!\n\n", RESET;
      exit();
    }

    my $n_count = (${$masked_seq_ref->{$sequence_name}->{sequence}} =~ tr/N/N/);

    my $percent_masked = ($n_count/$masked_length)*100;
    print BLUE, "\n\t\tA total of $percent_masked% of $sequence_name is repeat masked\n", RESET;
    print LOG "\n\t\tA total of $percent_masked% of $sequence_name is repeat masked\n";

    $unmasked_seq_ref->{$sequence_name}->{sequence} = ();
    $masked_seq_ref->{$sequence_name}->{sequence} = ();
  }
...
sub getRawProbes{
  ...
  my $um_seq_object_ref = $sequence_object_ref->{$chromosome}->{unmasked};
  my $m_seq_object_ref = $sequence_object_ref->{$chromosome}->{masked};

  my $start_pos = $region_object_ref->{$region_id}->{start};
  my $end_pos = $region_object_ref->{$region_id}->{end};

  #Get the source sequence to work with
  my $seq_ref = $um_seq_object_ref->{$chromosome}->{sequence};
  my $masked_seq_ref = $m_seq_object_ref->{$chromosome}->{sequence};
...
sub printProbeInfo{
  ...
  #Probe_Count\tProbeSet_ID\tRegion_ID\tSequence\tProbe_length\tProbe_Tm\tProbe_Type\tChromosome\tStart\tEnd\tmasked_bases
  ...
  foreach my $probe (sort {$a<=>$b} keys %{$probe_object_ref}){
   ...
   print OUTFILE "$current_probe_id\t$current_probeset_id\t$probe_object_ref->{$probe}->{region_id}\t$probe_object_ref->{$probe}->{sequence}\t$probe_object_ref->{$probe}->{length}\t$Tm_rounded\t$probe_object_ref->{$probe}->{chromosome}\t$probe_object_ref->{$probe}->{start_pos}\t$probe_object_ref->{$probe}->{end_pos}\t$probe_object_ref->{$probe}->{masked_bases}\n";
