...
main()
{
	...
	for (i = 0; i < NUM_MPS; i++)
	{
		...
		printf("ID = %d\n", m_o_p[i].employee_id);
		printf("Current Salary = $%.2lf\n", m_o_p[i].salary);
	
		increase_salary(&m_o_p[i]);
	
		printf("Incremented salary = $%.2f\n", m_o_p[i].salary);
		printf("\n");
	}
}
...
*  Also illustrates the usage of THE 'TO' OPERATOR ->
*  "(*emp_ptr).salary" and "emp_ptr->salary" are equivalent.
*/
void
increase_salary(Employee *emp_ptr)
{
	emp_ptr->salary += emp_ptr->salary * 0.2;
