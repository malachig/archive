...
my $expressed_gene_value = '';   #Number of reads for a gene required for a gene to be considered expressed/detected
...
GetOptions ('database=s'=>\$database,'server=s'=>\$server, 'user=s'=>\$user, 'password=s'=>\$password,
	    ...
	    'expressed_gene_value=i'=>\$expressed_gene_value, 'expressed_base_value=i'=>\$expressed_base_value,
	    'outfile=s'=>\$outfile);
...
print GREEN, "\n\nExample: coverageVsDepth_Gene-ExonicBase.pl  --database=ALEXA_hs_53_36o  --server=jango.bcgsc.ca  --user=username  --password=pwd  --read_records_dir=/projects/malachig/solexa/read_records/HS04391/ENST_v53/  --min_bit_score=48.1  --block_size=100000  --expressed_gene_value=10  --expressed_base_value=10  --outfile=/projects/malachig/solexa/read_records/HS04391/Summary/HS04391_Lanes1-23_Gene-ExonicBases_Coverage_10xCoverage.txt\n\n", RESET;

unless ($database && $server && $user && $password && $read_records_dir && $min_bit_score && ($block_size =~ /\d+/) && ($expressed_gene_value =~ /\d+/) && ($expressed_base_value =~ /\d+/) && $outfile){
...
print OUT "#expressed_gene_value = $expressed_gene_value\n";
...
print OUT "#MappedReads\tExpressedGenes\tExonCoverage\n";
...
my $temp_file = "$read_records_dir"."coverage_gene_exonic_temp.txt";
my $file_io_fails = 0;
my $fc = 0;
...
#Calculate the exon coverage for all the exons of those genes that were actually detected as expressed!
$total_exon_bases = 0;
my %genes_found;
foreach my $gene_id (keys %{$genes_ref}){
  if ($genes_ref->{$gene_id}->{quality_read_count} >= $expressed_gene_value){
    $genes_found{$gene_id}{tmp} = '';
  }
}

my $found_gene_count = keys %genes_found;
$| = 1; print BLUE, "\n\n Found a total of $found_gene_count genes.  Determining the coverage for these genes only", RESET; $| = 0;
print OUT "#Found a total of $found_gene_count genes.  Determining the coverage for these genes only\n";
...
sub summarizeCoverage{
  ...
  #Calculate the number of genes detected as expressed according to the user specified level
  my %genes_expressed;
  foreach my $gene_id (keys %{$genes_ref}){
    if ($genes_ref->{$gene_id}->{quality_read_count} >= $expressed_gene_value){
  ...
  my $gene_diff = $expressed_gene_count - $last_gene_count;
  $last_gene_count = $expressed_gene_count;
  ...
  $| = 1; print BLUE, "\n[READS: $grand_mapped_reads]\t[GENES: $expressed_gene_count ($gene_diff new)]\t[BASES: $bases_covered ($bases_diff new)]", RESET; $| = 0; 
  $| = 1; print BLUE, "\n\t[Elapsed Time = $seconds seconds]\t[% memory usage = $memory_usage]", RESET; $| = 0; 
  print OUT "$grand_mapped_reads\t$expressed_gene_count\t$bases_covered\n";
