...
GetOptions('dataname=s'=>\$dataname, 'feat_data_file=s'=>\$feat_data_file, 'feat_expressed_file=s'=>\$feat_expressed_file, 'group_member_lists=s'=>\$group_member_lists, 'results_dir=s'=>\$results_dir, 'short_name=s'=>\$short_name, 'image_type=s'=>\$image_type, 'comp_id=s'=>\$comp_id);
...
print GREEN, "\n\nUsage: calculateGroupDifferentialExpression.pl  --dataname=\"Basal_vs_BasalNorm_Gene\"  --feat_data_file=Matrix_GeneExpression_v53.txt.gz  --feat_expressed_file=Expressed_GeneExpression_v53.txt.gz  --group_member_lists=Basal:HCC1954,HCC3153,HCC1569,HCC70,HCC1500;BasalNorm:MCF10A,MCF10F --results_dir=/scratch/obig/ALEXA/analysis/figures_and_stats/DE/BCCL2/ENST_v53/  --short_name=Gene  --image_type=svg  --comp_id=Basal_vs_BasalNorm\n\n", RESET;

#Check user supplied options
unless ($dataname && $feat_data_file && $feat_expressed_file && $comp_id && $group_member_lists && $results_dir && $short_name && ($image_type =~ /tiff|jpeg|svg/i)){
...
#/global/home/groups/ac_cancerseq/ALEXA/analysis/batch_jobs/BCCL2/lawrencium/partition
#Unzip input matrix file if necessary.
my $temp_data_file = "$results_dir"."$short_name"."_"."$comp_id"."_"."temp_data_GDE.txt";
my $temp_expressed_file = "$results_dir"."$short_name"."_"."$comp_id"."_"."temp_expressed_GDE.txt";
...
my $r_script;
if($image_type =~ /svg/i){
  $r_script = "$script_dir/R_bin/calculateGroupDifferentialExpression_SVG.R";
}else{
  print RED, "\nImage type not currently supported\n\n", RESET;
  exit();
}

my $cmd_r = "$r_script $dataname $temp_data_file $temp_expressed_file $results_dir \"$short_name\" \"$group_member_lists\"";
print BLUE, "\n$cmd_r\n\n", RESET;
system($cmd_r);
