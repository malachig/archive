...
sub getAnnotations{
  ...
  my $gene_filter = 0;
  my @tmp = keys %{$types_list_ref};
  if ($gene_id_list_ref){
    $gene_filter = 1;
    my $gene_count = keys %{$gene_id_list_ref};
    print BLUE, "\n\nImporting annotations for $gene_count genes (types: @tmp)\n", RESET;
  }else{
    print BLUE, "\n\nImporting annotations for all genes (types: @tmp)\n", RESET;
  }
  ...
  if ($types_list_ref->{'Junction'} || $types_list_ref->{'KnownJunction'} || $types_list_ref->{'NovelJunction'}){
    my %junctions;
  ...
  if ($types_list_ref->{'Boundary'} || $types_list_ref->{'KnownBoundary'} || $types_list_ref->{'NovelBoundary'}){
    my %boundaries;
  ...
  #'Intergenic'
  my %intergenics;
  if ($types_list_ref->{'Intergenic'} || $types_list_ref->{'ActiveIntergenicRegion'} || $types_list_ref->{'SilentIntergenicRegion'}){
...
sub skipGene{
  my %args = @_;
  my $gene_list_ref = $args{'-gene_list'};
...
sub getExpressionData{
  ...
  my $types_list_ref = $args{'-types_list'};
  my $no_images = $args{'-no_images'};
  my %region_data_files;
  if ($args{'-data_files'}){%region_data_files= %{$args{'-data_files'}};}
  ...
  foreach my $library (sort {$a cmp $b} keys %{$paths_ref}){
    ...
    foreach my $data_dir (sort {$a cmp $b} keys %data_dirs){
      ...
      #Get all available data files (files must contain 'Expression' in their name)
    ...
    #If the region-by-region datafiles were supplied, use these instead
    if (%region_data_files){
      %data_files = %{$region_data_files{$library}{data_files}};
    }
    ...
    unless(defined($no_images)){
      print BLUE, "\n\n\tImporting images for library: $library ($name $stats_dir)", RESET;
...
sub getDifferentialExpressionData{
  ...
  foreach my $comparison (sort {$a cmp $b} keys %{$paths_ref}){
    ...
    my %data_files;
    #2.) Now get the actual differential expression files
    foreach my $data_dir (sort {$a cmp $b} keys %data_dirs){
      my $path =  $data_dirs{$data_dir}{path};
      my $summary_dir = "$path"."/";
    ...
    unless(defined($no_images)){
      print BLUE, "\n\n\tImporting images for comparison: $comparison ($comparison_name $stats_dir)", RESET;
...
sub getDifferentialSplicingData{
  ...
  foreach my $comparison (sort {$a cmp $b} keys %{$paths_ref}){
    ...
    my %data_files;
    #2.) Now get the actual alternative expression files
    foreach my $data_dir (sort {$a cmp $b} keys %data_dirs){
      my $path =  $data_dirs{$data_dir}{path};
      my $summary_dir = "$path"."/";
    ...
    unless(defined($no_images)){
      ...
      foreach my $data_type (sort {$a cmp $b} keys %data_files){
        ...
        my $scan = "ls $stats_dir"."*/*.svg";
        my @temp = `$scan`;
        my $string = "$comparison_name"."_"."$data_type"."_SI_Values_Cutoff_Hist.svg";
        foreach my $file (@temp){
          chomp($file);
          if ($file =~ /$string/){
            my $name = '';
...
sub generateDifferentialExpressionContent{
  ...
  foreach my $comparison (sort {$comparisons_ref->{$a}->{line_order} <=> $comparisons_ref->{$b}->{line_order}} keys %{$comparisons_ref}){
    ...
    foreach my $id (sort {$data_ref->{$a}->{rank} <=> $data_ref->{$b}->{rank}} keys %{$data_ref}){
      ...
      my $libA_name = $exp_paths_ref->{$libA}->{name};
      my $libB_name = $exp_paths_ref->{$libB}->{name};
  ...
  $div_count++;
  my $legend_div_name = "leg"."$div_count"."s";
  $content .= "\n<P CLASS=\"Indented12LR_s16_bold\">Significant differentially expressed $data_type features</P>";
  $content .= "\n<P CLASS=\"Indented12LR\"><a href=\"#\" rel=\"toggle[$legend_div_name]\" data-openimage=\"images/QuestionOff_icon.gif\" data-closedimage=\"images/QuestionOn_icon.gif\"><img src=\"images/QuestionOff_icon.gif\" border=\"0\" /></a></P>";
  $content .= "\n<div id=\"$legend_div_name\" style=\"width: 800px;\">";
...
sub generateDifferentialSplicingContent{
  ...
  foreach my $id (sort {$data_ref->{$a}->{rank} <=> $data_ref->{$b}->{rank}} keys %{$data_ref}){
    ...
    my $record = "<TD CLASS=\"$class\">$gene_name_link | $seq_name | $linkA | $linkB</TD><TD CLASS=\"Data2\">$si</TD><TD CLASS=\"Data2\">$gene_fold_change</TD><TD CLASS=\"Data2\">$seq_fold_change</TD><TD CLASS=\"Data2\">$reciprocity</TD><TD CLASS=\"Data2\">$percent_seq_log2_de</TD>";

    #If this is a junction include the exons-skipped with the seq name
    if ($data_type_comp =~ /Junction/){
       my $exons_skipped = $seq_a_ref->{$id}->{exons_skipped};
       $record = "<TD CLASS=\"$class\">$gene_name_link | $seq_name (S$exons_skipped) | $linkA | $linkB</TD><TD CLASS=\"Data2\">$si</TD><TD CLASS=\"Data2\">$gene_fold_change</TD><TD CLASS=\"Data2\">$seq_fold_change</TD><TD CLASS=\"Data2\">$reciprocity</TD><TD CLASS=\"Data2\">$percent_seq_log2_de</TD>";
    }
  ...
  #Print table header html
  my $link1 = "SI_"."$data_type_comp"."_"."$rank_terms[0]".".htm";
  my $link2 = "SI_"."$data_type_comp"."_"."$rank_terms[1]".".htm";
  my $link3 = "SI_"."$data_type_comp"."_"."$rank_terms[2]".".htm";
  my $link4 = "SI_"."$data_type_comp"."_"."$rank_terms[3]".".htm";
  my $link5 = "SI_"."$data_type_comp"."_"."$rank_terms[4]".".htm";
