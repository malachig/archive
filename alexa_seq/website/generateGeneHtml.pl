...
#Get partitioned expression files that correspond to the current genome region
my $data_files_ref;
if ($chr_filter){
  $data_files_ref = &getDataFiles('-expression_paths'=>$expression_paths_ref, '-chr'=>$chr_filter, '-region'=>$region_number, '-ensembl_version'=>$ensembl_version);
}
...
my $expression_data_ref = &getExpressionData('-paths'=>$expression_paths_ref, '-a_ref'=>$a_ref, '-ensembl_version'=>$ensembl_version, '-import_option'=>"all", '-types_list'=>\%types_list, '-data_files'=>$data_files_ref, '-no_images'=>1);

&createGeneDataFile('-expression_data_ref'=>$expression_data_ref, '-library_list'=>\%library_list, '-temp_dir'=>$temp_dir);


#Also import the differential expression data
my $de_data_ref = &getDifferentialExpressionData('-paths'=>$de_paths_ref, '-a_ref'=>$a_ref, '-ensembl_version'=>$ensembl_version, '-import_option'=>"filtered", '-types_list'=>\%types_list, '-no_images'=>1);

#Also import the alternative expression data
my $ae_data_ref = &getDifferentialSplicingData('-paths'=>$ae_paths_ref, '-a_ref'=>$a_ref, '-ensembl_version'=>$ensembl_version, '-import_option'=>"filtered", '-types_list'=>\%types_list, '-no_images'=>1);

my %comparison_list;
foreach my $data_type_comp (sort {$ae_data_ref->{$a}->{line_order} <=> $ae_data_ref->{$b}->{line_order}} keys %{$ae_data_ref}){
  my $comparison = $ae_data_ref->{$data_type_comp}->{comparison};
  my $comparison_name = $ae_paths_ref->{$comparison}->{name};
  $ae_data_ref->{$data_type_comp}->{comparison_name} = $comparison_name;
  $comparison_list{$comparison_name}{line_order} = $ae_data_ref->{$data_type_comp}->{line_order};
  $comparison_list{$comparison_name}{comparison_id} = $comparison;
}
...
foreach my $gene_id (sort {$a <=> $b} @gene_id_list){
  ...
  &buildGeneModel('-gene_id'=>$gene_id, '-a_ref'=>$a_ref, '-gene_seq_ref'=>$gene_seq_ref, '-types_list'=>\@model_types_list, 
                  '-output_file'=>$gene_model_file, '-de_data_ref'=>$de_data_ref, '-ae_data_ref'=>$ae_data_ref);
  ...
  #7-E.) Generate an HTML table to summarize the DE and AE features in more detail
  #For each comparison AND for each DE or AE feature, display the log2 difference and SI value 
  #Alignment option.  1=BLAST only   2=BLAST+BWA
  my $differential_table_content = &generateDifferentialTable('-a_ref'=>$a_ref, '-gene_expression_ref'=>$gene_expression_ref, '-types_list'=>\@types_list, '-comparison_list'=>\%comparison_list, '-gene_name'=>$gene_name);



  #Determine where the webpage for the current gene will be written to... define $web_path and $images_dir
...
<P CLASS=\"Indented12LR\"><a href="javascript:animatedcollapse.show(['box1s','box2h','box3h','box4h','box5h','box6h','box7h','box8h','box9h','leg1h','leg2h','leg3h'])"><IMG SRC="../../images/Plus_icon.gif" CLASS="Pic_unbordered"></a> | <a href="javascript:animatedcollapse.hide(['box1s','box2h','box3h','box4h','box5h','box6h','box7h','box8h','box9h','leg1h','leg2h','leg3h'])"><IMG SRC="../../images/Minus_icon.gif" CLASS="Pic_unbordered"></a> Show/Hide All Content</P>
...
<!-- Start of differential table section for $gene_name ($ensembl_g_id) -->
$differential_table_content
<!-- End of differential table section -->
...
sub getDataFiles{
  my %args = @_;
  my $expression_paths_ref = $args{'-expression_paths'};
  my $chr = $args{'-chr'};
  my $region = $args{'-region'};
  my $ensembl_version = $args{'-ensembl_version'};

  print BLUE, "\n\nGenerating file paths for chr: $chr and region: $region", RESET;
  my %types;
  $types{1}{feature_dir} = "ENST";
  $types{1}{name} = "Gene";
  $types{2}{feature_dir} = "ENST";
  $types{2}{name} = "ExonRegion";
  $types{3}{feature_dir} = "Transcripts";
  $types{3}{name} = "Transcript";
  $types{4}{feature_dir} = "Junctions";
  $types{4}{name} = "Junction";
  $types{5}{feature_dir} = "Boundaries";
  $types{5}{name} = "Boundary";
  $types{6}{feature_dir} = "Introns";
  $types{6}{name} = "Intron";
  $types{7}{feature_dir} = "Introns";
  $types{7}{name} = "ActiveIntronRegion";
  $types{8}{feature_dir} = "Introns";
  $types{8}{name} = "SilentIntronRegion";
  $types{9}{feature_dir} = "Intergenics";
  $types{9}{name} = "Intergenic";
  $types{10}{feature_dir} = "Intergenics";
  $types{10}{name} = "ActiveIntergenicRegion";
  $types{11}{feature_dir} = "Intergenics";
  $types{11}{name} = "SilentIntergenicRegion";
  
  my %lib_files;

  #For each library, define paths to expression values for a specific region for each expression data type
  my $missing_files = 0;
  foreach my $library (sort {$expression_paths_ref->{$a}->{line_order} <=> $expression_paths_ref->{$b}->{line_order}} keys %{$expression_paths_ref}){
    my $data_path = $expression_paths_ref->{$library}->{data_path};
    my %files;

    foreach my $tc (sort {$a <=> $b} keys %types){
      my $feature_dir = $types{$tc}{feature_dir};
      my $feature_dir_full = "$feature_dir"."_v"."$ensembl_version";
      my $type_name = $types{$tc}{name};
      my $results_file = "$data_path"."$feature_dir_full"."/Summary/results/chr$chr"."_"."$region"."_"."$type_name"."Expression.txt";

      #Special case for transcript expression file
      if ($type_name eq "Transcript"){
        $results_file = "$data_path"."$feature_dir_full"."/Summary/$library"."_TranscriptExpression_v"."$ensembl_version".".txt";
      }

      #Special case for gene expression file
      if ($type_name eq "Gene"){
        $results_file = "$data_path"."$feature_dir_full"."/Summary/$library"."_GeneExpression_v"."$ensembl_version".".txt";
      }

      $files{$type_name}{path} = $results_file;
      unless (-e $results_file){
        $missing_files++;
      }
      #print YELLOW, "\n$results_file", RESET;
    }
    $lib_files{$library}{data_files} = \%files;
  }
  if ($missing_files > 0){
    %lib_files = ();
  }
...
sub buildExpressionObject{
  ...
  #Check each feature to see if it was identified as differentially and alternatively expressed in at least one comparison - if so mark it
  foreach my $ec (keys %expression){
    my $id = $expression{$ec}{id};
    my $type = $expression{$ec}{data_type};

    my $comparisons_ref = $de_data_ref->{$type}->{comparisons};
    foreach my $comparison (keys %{$comparisons_ref}){
      my $data_ref = $comparisons_ref->{$comparison}->{data};
      if ($data_ref->{$id}){
        $expression{$ec}{is_de} = 1;
      }
    }
...
sub buildGeneModel{
  ...
  my $gene_seq_ref = $args{'-gene_seq_ref'};
  my @types_list = @{$args{'-types_list'}};
  my $de_data_ref = $args{'-de_data_ref'};
  my $ae_data_ref = $args{'-ae_data_ref'};
...
###########################################################################################################################
#Generate an HTML table to summarize the differential and alternative expression values                                   #
###########################################################################################################################
sub generateDifferentialTable{
  my %args = @_;
  my $a_ref = $args{'-a_ref'};
  my $gene_expression_ref = $args{'-gene_expression_ref'};
  my @types_list = @{$args{'-types_list'}};
  my %comparison_list = %{$args{'-comparison_list'}};
  my $gene_name = $args{'-gene_name'};

  #Display columns as follows:
  #Feature ID, First Comparison (log2diff | SI) ... Last Comparison (log2diff | SI)

  #Open table
  my $table_content = "\n<P CLASS=\"Indented12LR_s16_Bold\">DE and AE features of '<i>$gene_name</i>'</P>";
  ...
  $table_content .= "\n<P CLASS=\"Indented24LR_s16\">The following table displays the log2 difference and splicing index (SI) values at the level of the gene and for each known transcript. Following these values are the log2 difference and SI values for each individual sequence feature (exon, junction, etc.) that was found to be significantly differentially or alternatively expressed.  Features that did not pass the significance threshhold are not displayed.  For more details on each feature, including absolute expression levels in each library, refer to the expression table above.  To visualize the differences in expression refer to the figures below.</P><BR>";
  $table_content .= "\n</div>";

  $table_content .= "\n<TABLE CLASS=\"Data2\">";

  #Print header row
  $table_content .= "\n  <TR><TD CLASS=\"Head3\">FID</TD><TD CLASS=\"Head3\">Name</TD><TD CLASS=\"Head3\">Type</TD>";
  foreach my $comparison_name (sort {$comparison_list{$a}{line_order} <=> $comparison_list{$b}{line_order}} keys %comparison_list){
    $table_content .= "<TD CLASS = \"Head3\">$comparison_name<br>(Log2 Diff | SI Value)</TD>";
  }
  $table_content .= "\n  </TR>";
  ...
  foreach my $process_group (@process_groups){
    foreach my $ec (sort {$gene_expression_ref->{$a}->{order} <=> $gene_expression_ref->{$b}->{order}} keys %{$gene_expression_ref}){
      ...
      #Unless the feature is DE or AE, skip it
      unless ($gene_expression_ref->{$ec}->{is_de} || $gene_expression_ref->{$ec}->{is_ae}){
        next();
      }

      #Get basic info for this feature from the gene expression object (expressed features for the current gene only)
      my $id = $gene_expression_ref->{$ec}->{id};
      my $seq_name = $gene_expression_ref->{$ec}->{seq_name};
      my $type = $gene_expression_ref->{$ec}->{data_type};
      my $type_name = $gene_expression_ref->{$ec}->{type_name};
      ...
      #Get the DE and SI values for the current feature for each comparison
      my $de_comparisons_ref = $de_data_ref->{$type}->{comparisons};
      my $data_entries = '';
      foreach my $comparison_name (sort {$comparison_list{$a}{line_order} <=> $comparison_list{$b}{line_order}} keys %comparison_list){
        my $comparison = $comparison_list{$comparison_name}{comparison_id};
        ...
        my $libA_name = $expression_paths_ref->{$libA}->{name};
        my $libB_name = $expression_paths_ref->{$libB}->{name};
        my $linkA = "$libA_name";
        my $linkB = "$libB_name";
        if ($chromosome =~ /^\d$|^\d{2}$|^X$|^Y$|^M$/){
          $linkA = "<A HREF=\"$ucsc_link_A\" TARGET=\"_blank\">libA</A>";
          $linkB = "<A HREF=\"$ucsc_link_B\" TARGET=\"_blank\">libB</A>";
        }

        #DE value
        my $comparisons_ref = $de_data_ref->{$type}->{comparisons};
        my $data_ref1 = $comparisons_ref->{$comparison}->{data};
        my $log2_diff = "-";
        if ($data_ref1->{$id}){
          $log2_diff = $data_ref1->{$id}->{log2_diff};
          if ($log2_diff =~ /\d+/){
            $log2_diff = sprintf("%.2f", $log2_diff);
          }
        }

        #SI value
        my $data_type_comp = "$type"."_"."$comparison";
        my $data_ref2 = $ae_data_ref->{$data_type_comp}->{data};
        my $si = "-";
        if ($data_ref2){
          if ($data_ref2->{$id}){
            $si = $data_ref2->{$id}->{si};
        ...
        my $string = "$log2_diff | $si";
        $data_entries .= "<TD CLASS=\"Data4\">$string<br>($linkA vs $linkB)</TD>";
      }

      my $class = "Data2";
      $table_content .= "\n  <TR><TD CLASS=\"Data2\">$fid</TD><TD CLASS=\"Data3\">$seq_name</TD><TD CLASS=\"Data1\">$type_name</TD>$data_entries";
...
sub generateExpressionFigure{
  ...
  #First figure out if there are any expressed features
  ...
  #Only print output files and generate graphs if some feature was expressed in some library
  if ($expressed_data_count > 0){
    ...
    print OUT "Order\tSeqName\tSeqType\tIsDE\tIsAE\t$lib_string\n";
    ...
    foreach my $process_group (@process_groups){
      ...
      foreach my $ec (sort {$gene_expression_ref->{$a}->{order} <=> $gene_expression_ref->{$b}->{order}} keys %{$gene_expression_ref}){
        ...
        push(@data, $gene_expression_ref->{$ec}->{is_de});
    ...
    $figure_content .= "\n<P CLASS=\"Indented12LR\"><a href=\"#\" rel=\"toggle[box9h]\" data-openimage=\"../../images/Minus_icon.gif\" data-closedimage=\"../../images/Plus_icon.gif\"><img src=\"../../images/Plus_icon.gif\" border=\"0\" /></a></P>";
    $figure_content .= "<div id=\"box9h\">";

    #Define the figure height and width (this should match what was used in the R script)
    my $image_width = 1000;
    my $image_height = (400*(($lib_count*2)+1));


    #Using SVGs
    #Run R script
    my $r_script = "$script_dir"."/R_bin/geneFigure_SVG.R";

    #$r_cmd = "$r_script $all_genes_data_file $single_gene_data_file $gene_model_file \"$gene_name\" \"$ensg_id\" $gene_id $images_dir \"$intragenic_cutoffs_string\" \"$intergenic_cutoffs_string\" $grand_cds_start $grand_cds_end";
    $r_cmd = "$r_script $all_genes_data_file $single_gene_data_file $gene_model_file \"$gene_name\" \"$ensg_id\" $gene_id $images_dir \"$intragenic_cutoffs_string\" \"$intergenic_cutoffs_string\" $grand_cds_start $grand_cds_end 2>/dev/null";
    ...
    $figure_content .= "\n<P CLASS=\"Indented24LR\"><a href=\"#\" rel=\"toggle[leg3h]\" data-openimage=\"../../images/QuestionOff_icon.gif\" data-closedimage=\"../../images/QuestionOn_icon.gif\"><img src=\"../../images/QuestionOn_icon.gif\" border=\"0\" /></a></P>";
    $figure_content .= "\n<div id=\"leg3h\" style=\"width: 1000px;\">";
