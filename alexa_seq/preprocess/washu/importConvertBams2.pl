...
unless ($working_dir && $library_name && $bam_file){
  print RED, "\n\nParameters missing\n\n", RESET;
...
unless (-e $working_dir && -d $working_dir){
  print RED, "\n\nCould not find specified working directory\n\n";
  exit();
}
...
if ($bam_file =~ /\/(\w+)\_(\d+)\.bam$/){
  $flowcell = $1;
  $lane = $2;
}else{
  print RED, "\n\nCould not determine flowcell and lane from bam file path: $bam_file\n\n", RESET;
  exit();
}

if ($lane eq 'unknown' || $flowcell eq 'unknown'){
  print RED, "wtf.. drink more coffee and check script again...";
  exit();
}
