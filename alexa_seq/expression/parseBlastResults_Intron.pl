...
#The purpose of this script is to parse blast results files for Solexa Paired Reads that were BLASTed against a database of human Introns

#NOTE that these are the result of mapping individual reads of a pair to a database of known and theoretical EnsEMBL introns
#- Reads which hit a repeat, exon, junction or boundary of a known transcript should have already been captured by mapping directly to the transcripts, junctions and boundaries
#- This analysis should capture reads that hit entirely within the introns of know EnsEMBL genes
#- Since introns are often quite large, it may be possible to map reads as a pair in some cases - but this is not required
...
GetOptions ('intron_db=s'=>\$intron_db, 'read_records_infile=s'=>\$read_records_infile, 'blast_results_dir=s'=>\$blast_results_dir,
...
print GREEN, "\n\tSpecify an annotated intron database file using: --intron_db", RESET;
...
print GREEN, "\n\nExample: parseBlastResults_Intron.pl  --intron_db=/projects/malachig/sequence_databases/hs_49_36k/ensembl_introns_hs_49_36k/introns_annotated.txt.gz  --read_records_infile=/projects/malachig/solexa/read_records/HS04391/20821AAXX_Lane1.txt.gz  --blast_results_dir=/projects/malachig/solexa/blast_results/HS04391/20821AAXX_Lane1/ensembl_introns_v49/  --reads_analyzed_per_block=125000  --total_reads_analyzed=5738676  --working_dir=/projects/malachig/solexa/blast_results/HS04391/20821AAXX_Lane1/temp/  --summary_outfile=/projects/malachig/solexa/read_records/HS04391/Introns_v49/20821AAXX_Lane1_Introns_v49.txt  --min_bit_score=48.1  --upper_bit_score=60.0  --min_align_length=32  --logfile=/projects/malachig/solexa/logs/HS04391/20821AAXX_Lane1/parseIntronBlastResults_LOG.txt\n\n", RESET;

unless ($intron_db && $read_records_infile && $blast_results_dir && $reads_analyzed_per_block && $total_reads_analyzed && $working_dir && $summary_outfile && $min_bit_score && $upper_bit_score && $min_align_length && $logfile){
...
unless (-e $intron_db && ($intron_db =~ /\.gz/)){
  print RED, "\nAnnotation database file: $intron_db does not exist or is not compressed!!\n\n", RESET;
  exit();
}
...
#1.) Create a map of intron_IDs to chromosome coordinates by parsing the intron annotation database file.
#    - This hash will be needed to calculate the chromosome coordinates for hits to the intron sequences
my %introns;
open(INTRON_DB, "zcat $intron_db |") || die "\nCould not open intron db file: $intron_db\n\n";
my $intron_count = 0;
my $header = 1;
my %columns;
...
while(<INTRON_DB>){
  ...
  my $start = $line[$columns{'Unit1_start'}{column_pos}];
  my $end = $line[$columns{'Unit1_end'}{column_pos}];
  ...
  $introns{$intron_id}{start} = $start;
  $introns{$intron_id}{end} = $end;
  $introns{$intron_id}{start_chr} = $start_chr;
  $introns{$intron_id}{end_chr} = $end_chr;
  $introns{$intron_id}{size} = ($end-$start)+1;
...
my $top_hits_stored = &parseBlastFiles('-seq_type'=>$seq_type, '-blast_results_dir'=>$blast_results_dir, '-top_hits_db'=>\%top_hits_db, '-min_bit_score'=>$min_bit_score, 
                                       ...
                                       '-log_file_handle'=>$log_fh, '-seq_db'=>\%introns);
