...
#The purpose of this script is to parse blast results files for Solexa Paired Reads that were BLASTed against a database of human Exon-Exon Junctions or Exon Boundaries

#NOTE that these are the result of mapping individual reads of a pair to a database of known and theoretical EnsEMBL exon-exon junctions and exon boundaries
#- Reads which hit a junction or boundary of a known transcript will have already been captured by mapping directly to the transcripts
#- Some read hits here will therefore be redundant to those in the transcript mappings
#- This is just a simple way of getting a read-out for every junction or boundary
#- Since we are mapping to junctions or boundaries only in this case, it doesnt make sense to consider read pairing info to resolve ambiguities
...
#    - Store the best exon-exon junction or exon boundary hits
...
my $summary_outfile = '';
my $logfile = '';
my $seq_type = '';         #Whether exon-exon junctions or exon boundaries are being processed
my $seq_size = '';
...
GetOptions ('read_records_infile=s'=>\$read_records_infile, 'blast_results_dir=s'=>\$blast_results_dir,
	    ...
	    'min_bit_score=f'=>\$min_bit_score, 'upper_bit_score=f'=>\$upper_bit_score,
	    'logfile=s'=>\$logfile, 'seq_type=s'=>\$seq_type, 'seq_size=i'=>\$seq_size);

#Provide instruction to the user
print GREEN, "\n\nUsage:", RESET;
print GREEN, "\n\tThis parses blast results for Solexa paired reads BLASTED against a database of exon-exon junctions or exon boundaries", RESET;
...
print GREEN, "\n\tSpecify whether you are processing BLAST results for exon-exon junctions or exon boundaries using:  --seq_type=junction OR --seq_type=boundary", RESET;
print GREEN, "\n\tSpecify the junction/boundary sequence size using:  --seq_size", RESET;

print GREEN, "\n\nExample: parseExonJunctionBlastResults.pl  --read_records_infile=/projects/malachig/solexa/read_records/HS04391/20821AAXX_Lane1.txt  --blast_results_dir=/projects/malachig/solexa/blast_results/HS04391/20821AAXX_Lane1/ensembl_junctions_v49/  --reads_analyzed_per_block=125000  --total_reads_analyzed=5738676  --working_dir=/projects/malachig/solexa/blast_results/HS04391/20821AAXX_Lane1/temp/  --summary_outfile=/projects/malachig/solexa/read_records/HS04391/Junctions_v49/20821AAXX_Lane1_Junctions_v49.txt  --min_bit_score=69.9  --upper_bit_score=71.9  --logfile=/projects/malachig/solexa/logs/HS04391/20821AAXX_Lane1/parseExonJunctionBlastResults_LOG.txt  --seq_type=junction  --seq_size=62\n\n", RESET;

unless ($read_records_infile && $blast_results_dir && $reads_analyzed_per_block && $total_reads_analyzed && $working_dir && $min_bit_score && $upper_bit_score && $summary_outfile && $logfile && $seq_type && $seq_size){
  print RED, "\nRequired input parameter(s) missing\n\n", RESET;
  exit();
}

my $unambiguous_label;
my $ambiguous_label;
my $seq_id_name;
if ($seq_type =~ /junction|boundary/i){
  if ($seq_type =~ /junction/i){
    $unambiguous_label = "NOVEL_JUNCTION_U";
    $ambiguous_label = "NOVEL_JUNCTION_A";
    $seq_id_name = "Junction_ID";
  }elsif($seq_type =~ /boundary/i){
    $unambiguous_label = "NOVEL_BOUNDARY_U";
    $ambiguous_label = "NOVEL_BOUNDARY_A";
...
my $novel_seq_u_count = 0;
my $novel_seq_a_count = 0;


my $library_name;
if ($read_records_infile =~ /\/(\w+_Lane\d+)\.txt\.gz$/){
  $library_name = $1;
}else{
...
&parseBlastFiles('-blast_results_dir'=>$blast_results_dir, '-working_dir'=>$working_dir);
...
#Create a summary file by joining selected data from the top hits file to selected data from the read records file
#Also update the read records file
...
my $cmd_rm1 = "rm -f $top_hits_db_file";
system($cmd_rm1);
$message = &memoryUsage();
print YELLOW, "\n\n$message", RESET;
print LOG "\n\n$message";
...
sub parseBlastFiles{
  ...
  #Info to store: "Read_ID\tHit_Type\tSubject_ID\tAlignmentLength\tPercentIdentity\tBitScore\tSubjectStart\tSubjectEnd";
...
############################################################################################################################################################
#Join top hit records to read records
############################################################################################################################################################
sub joinRecords{
  my %args = @_;
  my $read_records_infile = $args{'-read_records_infile'};
  ...
  my $counter = 0;
  my $header = 1;
  open(READS, "zcat $read_records_infile |") || die "\nCould not open read records infile: $read_records_infile\n\n";
  while(<READS>){
    $counter++;
    chomp($_);
    if ($header == 1){
  ...
  print SUMMARY "Read_ID\tHitType\t$seq_id_name\tAlignmentLength\tPercentIdentity\tBitScore\tStart\tEnd\tStrand\n";
  ...
  while (my ($read_pair_id, $read_pair_string) = each %read_pairs_db){
	...
	if ($start > $end){
      ...
      print SUMMARY "$read1_id\t$hit_type\t$seq_id\t$align_length\t$percent_id\t$bit_score\t$start\t$end\t$strand\n";
      ...
      #Possible status is '$unambiguous_label' or '$ambiguous_label'

      #If a read has already been assigned to '$unambiguous_label' or '$ambiguous_label', start by resetting these to 'Unassigned'
      if ($read1_status eq "$unambiguous_label" || $read1_status eq "$ambiguous_label"){
	...
	if (($hit_type eq "Top_Hit") && ($bit_score >= $upper_bit_score)){
	  $novel_seq_u_count++;
	  $read1_status = "$unambiguous_label";
	}elsif(($hit_type eq "Ambiguous") && ($bit_score >= $upper_bit_score)){
	  $novel_seq_a_count++;
	  $read1_status = "$ambiguous_label";
	}
      }
    }
	...
	if ($start > $end){
      ...
      print SUMMARY "$read2_id\t$hit_type\t$seq_id\t$align_length\t$percent_id\t$bit_score\t$start\t$end\t$strand\n";
	
      #If the upper bit score criteria is met and the status of this read is still 'Unassigned' in the read record file, change the status
      #Possible status is '$unambiguous_label' or '$ambiguous_label'

      #If a read has already been assigned to 'Repeat_U' or 'Repeat_A', start by resetting these to 'Unassigned'
      if ($read2_status eq "$unambiguous_label" || $read2_status eq "$ambiguous_label"){
	...
	if (($hit_type eq "Top_Hit") && ($bit_score >= $upper_bit_score)){
	  $novel_seq_u_count++;
	  $read2_status = "$unambiguous_label";
	}elsif(($hit_type eq "Ambiguous") && ($bit_score >= $upper_bit_score)){
	  $novel_seq_a_count++;
	  $read2_status = "$ambiguous_label";
	}
      }
    }
  ...
  my $percent_u = sprintf("%.2f", (($novel_seq_u_count/$read_count)*100));
  my $percent_a = sprintf("%.2f", (($novel_seq_a_count/$read_count)*100));

  print BLUE, "\n\nAssigned $novel_seq_u_count reads ($percent_u%) to the status '$unambiguous_label', and $novel_seq_a_count ($percent_a%) reads to the status '$ambiguous_label'\n\n", RESET;
  print LOG "\n\nAssigned $novel_seq_u_count reads ($percent_u%) to the status '$unambiguous_label', and $novel_seq_a_count ($percent_a%) reads to the status '$ambiguous_label'\n\n";
