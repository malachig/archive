...
foreach my $tr (keys %trs) {
	...
	if($trs{$tr}{exon_num} == 1) {
		print "$trs{$tr}{coords}{chr}:$trs{$tr}{exons}[0]{start}-$trs{$tr}{exons}[0]{end}\t$trs{$tr}{coords}{strand}\tse\t$tr\t$FPKM\n";
	} elsif($trs{$tr}{exon_num} > 1) {
		print "$trs{$tr}{coords}{chr}:$trs{$tr}{exons}[0]{start}-$trs{$tr}{exons}[0]{end}\t$trs{$tr}{coords}{strand}\tmeUS\t$tr\t$FPKM\n";
		for(my $i = 1; $i < $trs{$tr}{exon_num} - 1; $i++) {
			print "$trs{$tr}{coords}{chr}:$trs{$tr}{exons}[$i]{start}-$trs{$tr}{exons}[$i]{end}\t$trs{$tr}{coords}{strand}\tmeM\t$tr\t$FPKM\n";
		}
		print "$trs{$tr}{coords}{chr}:$trs{$tr}{exons}[$trs{$tr}{exon_num}-1]{start}-$trs{$tr}{exons}[$trs{$tr}{exon_num}-1]{end}\t$trs{$tr}{coords}{strand}\tmeDS\t$tr\t$FPKM\n";
	}
}
