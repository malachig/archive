...
GetOptions('database=s'=>\$database,'server=s'=>\$server, 'user=s'=>\$user, 'password=s'=>\$password, 'annotation_dir=s'=>\$annotation_dir, 
           ...
           'gde_import_option=s'=>\$gde_import_option);
...
print GREEN, "\n\tIf you have have group comparisons involving small numbers in each group you can turn off multiple testing correction using: --gde_import_option=[filtered | all | noMTC]", RESET;
...
if ($gde_import_option){
  unless ($gde_import_option =~ /filtered|all|noMTC/){
    print RED, "\n\n--gde_import_option must be one of: filtered|all|noMTC\n\n", RESET;
    exit();
  }
}
unless (-e $project_config_file){
  print RED, "\n\nCould not find project config file: $project_config_file\n\n", RESET;
  exit();
}
...
#my @types_list = qw (Transcript); #Use for debugging purposes
...
my $a_ref = &getAnnotations('-annotation_dir'=>$annotation_dir, '-dbh'=>$alexa_dbh, '-database'=>$database, '-types_list'=>\%types_list, '-junction_seq_size'=>$junction_seq_size, '-boundary_seq_size'=>$boundary_seq_size);
...
my $expression_data_ref = &getExpressionData('-paths'=>$expression_paths_ref, '-a_ref'=>$a_ref, '-ensembl_version'=>$ensembl_version, '-import_option'=>"expressed", '-types_list'=>\%types_list);
...
#Import group differential expression data
my $import_option = "filtered";
if ($gde_import_option){
  $import_option = $gde_import_option;
}
my $gde_paths_ref = $data_paths_ref->{'GroupDifferentialExpression'}->{paths};
my $gde_data_ref = &getDifferentialExpressionData('-paths'=>$gde_paths_ref, '-a_ref'=>$a_ref, '-ensembl_version'=>$ensembl_version, '-import_option'=>$import_option, '-types_list'=>\%types_list);
...
foreach my $data_type (sort {$expression_data_ref->{$a}->{order} <=> $expression_data_ref->{$b}->{order}} keys %{$expression_data_ref}){
  ...
  my @comparison_names;
  my $comp_count = 0;
  my $group_comp_count = 0;

  #Process differential expression values if the data type was already defined for expression values and is defined in the DE object
  my $content_ref_DE;
  if ($de_data_ref->{$data_type}){
    ...
    $content_ref_DE = &generateDifferentialExpressionContent('-data_type'=>$data_type, '-data_ref'=>$comparisons_ref, '-a_ref'=>$a_ref, '-de_paths'=>$de_paths_ref, '-exp_paths'=>$expression_paths_ref, '-web_images_dir'=>$web_images_dir, '-ucsc_build'=>$ucsc_build, '-track_url'=>$track_url, '-content_type'=>"DE", '-div_count_start'=>0);
  }

  #Process GROUP differential expression values if the data type was already defined for expression values and is defined in the DE object
  my $content_ref_GDE;
  if ($gde_data_ref->{$data_type}){
    my $comparisons_ref = $gde_data_ref->{$data_type}->{comparisons};
    my $libraries_ref = $expression_data_ref->{$data_type}->{libraries};
    ...
    foreach my $comparison (sort {$comparisons_ref->{$a}->{line_order} <=> $comparisons_ref->{$b}->{line_order}} keys %{$comparisons_ref}){
      ...
      my $comparison_name = $gde_paths_ref->{$comparison}->{name};
      push(@comparison_names, $comparison_name);
      print BLUE, "\n\tProcessing comparison: $comparison", RESET;
      ...
      #Rank the data
      my $assigned_ranks = &rankData('-data_ref'=>$data_ref, '-rank_variable'=>'q_value', '-descending'=>0, '-absolute'=>1);
      print BLUE, "\n\t\tAssigned ranks for $assigned_ranks records", RESET;
    }
    $content_ref_GDE = &generateDifferentialExpressionContent('-data_type'=>$data_type, '-data_ref'=>$comparisons_ref, '-a_ref'=>$a_ref, '-de_paths'=>$gde_paths_ref, '-exp_paths'=>$expression_paths_ref, '-web_images_dir'=>$web_images_dir, '-ucsc_build'=>$ucsc_build, '-track_url'=>$track_url, '-content_type'=>"GDE", '-div_count_start'=>$comp_count);
  }

  #Dereference, combine and re-reference as needed
  my $content_ref;
  if ($de_data_ref->{$data_type}){
    $content_ref=$content_ref_DE;
  }
  if ($gde_data_ref->{$data_type}){
    $content_ref=$content_ref_GDE;
  }
  if ($de_data_ref->{$data_type} && $gde_data_ref->{$data_type}){
    my $content = ${$content_ref_GDE} . ${$content_ref_DE};
    $content_ref = \$content;
  }
  ...
  if ($content_ref){
    ...
    my $web_path = "$web_root"."$web_name";
    my $divs_needed = ($comp_count+$group_comp_count)+1;
...
#Import group differential splicing data
my $gsi_paths_ref = $data_paths_ref->{'GroupDifferentialSplicing'}->{paths};
my $gsi_data_ref = &getDifferentialSplicingData('-paths'=>$gsi_paths_ref, '-a_ref'=>$a_ref, '-ensembl_version'=>$ensembl_version, '-import_option'=>"filtered", '-types_list'=>\%types_list);
...
#need to create two sections here, one for SI and one for GSI
#More complicated because there is a different page for every comparison and type combination
...
foreach my $data_type_comp (sort {$si_data_ref->{$a}->{line_order} <=> $si_data_ref->{$b}->{line_order}} keys %{$si_data_ref}){
  ...
  foreach my $rank_variable (@rank_terms){
    ...
    my $content_ref = &generateDifferentialSplicingContent('-data_type_comp'=>$data_type_comp, '-si_data_ref'=>$si_data_ref, '-a_ref'=>$a_ref, '-si_paths'=>$si_paths_ref, '-exp_paths'=>$expression_paths_ref,'-web_images_dir'=>$web_images_dir, '-ucsc_build'=>$ucsc_build, '-track_url'=>$track_url, '-rank_terms'=>\@rank_terms, '-content_type'=>"SI");
...
#GSI Comparisons
foreach my $data_type_comp (sort {$gsi_data_ref->{$a}->{line_order} <=> $gsi_data_ref->{$b}->{line_order}} keys %{$gsi_data_ref}){
  print BLUE, "\n\nProcessing data type: $data_type_comp", RESET;
 
  my $data_ref = $gsi_data_ref->{$data_type_comp}->{data};
  my $comparison = $gsi_data_ref->{$data_type_comp}->{comparison};
  my $data_type = $gsi_data_ref->{$data_type_comp}->{data_type};
  ...
  my $comparison_name = $gsi_paths_ref->{$comparison}->{name};
  print BLUE, "\n\tProcessing data_type comparison: $data_type_comp ($comparison_name)", RESET;

  #Rank using several different metrics for the same lists of events
  my @rank_terms = qw (si gene_fold_change seq_fold_change reciprocity percent_seq_log2_de q_value);
  ...
  foreach my $rank_variable (@rank_terms){
    ...
    #Rank the data
    my $descending;
    if ($rank_variable eq "q_value"){$descending=0;}else{$descending=1;}
    my $assigned_ranks = &rankData('-data_ref'=>$data_ref, '-rank_variable'=>$rank_variable, '-descending'=>$descending, '-absolute'=>1);
    print BLUE, "\n\t\t\tAssigned ranks for $assigned_ranks records", RESET;

    my $content_ref = &generateDifferentialSplicingContent('-data_type_comp'=>$data_type_comp, '-si_data_ref'=>$gsi_data_ref, '-a_ref'=>$a_ref, '-si_paths'=>$gsi_paths_ref, '-exp_paths'=>$expression_paths_ref, '-web_images_dir'=>$web_images_dir, '-ucsc_build'=>$ucsc_build, '-track_url'=>$track_url, '-rank_terms'=>\@rank_terms, '-content_type'=>"GSI");
    ...
    my $web_name = "GSI_"."$data_type_comp"."_"."$rank_variable".".htm";
    my $web_path = "$web_root"."$web_name";
    &writePage('-path'=>$web_path, '-title'=>$title, '-content'=>$content_ref, '-css_path'=>"../ALEXA2.css", '-alexa_home_path'=>"$alexa_home_path", '-alexa_seq_home_path'=>"$alexa_seq_path", '-summary_path'=>"Summary.htm", '-genes_path'=>"genes/index.html",  '-search_path'=>$search_page_url, '-meta_description'=>$meta_description, '-meta_keywords'=>$meta_keywords, '-google_analytics'=>1, '-google_analytics_id'=>$google_analytics_id, '-collapse_div_script'=>'../animatedcollapse.js', '-jquery_min_script'=>'../jquery.min.js', '-div_count'=>5);
  }

  my $web_name = "GSI_"."$data_type_comp"."_si.htm";
  my $web_path = "$web_root"."$web_name";

  $gsi_data_ref->{$data_type_comp}->{web_page_name} = $web_name;
  $gsi_data_ref->{$data_type_comp}->{web_page_path} = $web_path;
}
...
foreach my $data_type_comp (sort {$si_data_ref->{$a}->{line_order} <=> $si_data_ref->{$b}->{line_order}} keys %{$si_data_ref}){
  my $comparison = $si_data_ref->{$data_type_comp}->{comparison};
  my $comparison_name = $si_paths_ref->{$comparison}->{name};
  $si_data_ref->{$data_type_comp}->{comparison_name} = $comparison_name;
  $comp_list{$comparison_name}{line_order} = $si_data_ref->{$data_type_comp}->{line_order};
...
foreach my $data_type_comp (sort {$gsi_data_ref->{$a}->{line_order} <=> $gsi_data_ref->{$b}->{line_order}} keys %{$gsi_data_ref}){
  my $comparison = $gsi_data_ref->{$data_type_comp}->{comparison};
  my $comparison_name = $gsi_paths_ref->{$comparison}->{name};
  $gsi_data_ref->{$data_type_comp}->{comparison_name} = $comparison_name;
  $group_comp_list{$comparison_name}{line_order} = $gsi_data_ref->{$data_type_comp}->{line_order};
  $group_comp_list{$comparison_name}{comparison_id} = $comparison;
}
foreach my $group_comparison_name (sort {$group_comp_list{$a}{line_order} <=> $group_comp_list{$b}{line_order}} keys %group_comp_list){
  $splicing_heads = "$splicing_heads"."<TD CLASS = \"Head3\">Alternative Expression<BR>($group_comparison_name)</TD>";
}

#Create links to library summary pages
my $library_summary_links = ""; 
foreach my $library (sort {$library_list{$a}->{order} <=> $library_list{$b}->{order}} keys %library_list){
  my $library_name = $library_list{$library}{name};
...
if (-e $results_package_path){
  print BLUE, "\n\t\tFound results package: $results_package\n\n", RESET;
}else{
  print YELLOW, "\n\t\tResults package missing: $results_package_path\n\n", RESET;
...
foreach my $current_data_type (@types_list){
  ...
  my $level = $expression_data_ref->{$current_data_type}->{level};
  my $td_class = "Head1";
  if ($level == 2){
    $td_class = "Head2";
  }
  ...
  foreach my $data_type (sort {$a cmp $b} keys %{$expression_data_ref}){
    unless($data_type eq $current_data_type){
      next();
    }
    my $page_name = $expression_data_ref->{$data_type}->{web_page_name};
    my $record_count = &commify($expression_data_ref->{$data_type}->{record_count});
  ...
  foreach my $data_type (sort {$a cmp $b} keys %{$de_data_ref}){
    unless($data_type eq $current_data_type){
      next();
    }
    my $page_name = $de_data_ref->{$data_type}->{web_page_name};
    my $record_count_DE = 0;
    my $record_count_GDE = 0;
    if ($de_data_ref->{$data_type}){
      if ($de_data_ref->{$data_type}->{record_count}){
        $record_count_DE = $de_data_ref->{$data_type}->{record_count};
      }
    }
    if($gde_data_ref->{$data_type}){
      if ($gde_data_ref->{$data_type}->{record_count}){
        $record_count_GDE = $gde_data_ref->{$data_type}->{record_count};
      }
    }
    my $record_count=&commify($record_count_DE+$record_count_GDE);
  ...
  foreach my $data_type_comp (sort {$si_data_ref->{$a}->{line_order} <=> $si_data_ref->{$b}->{line_order}} keys %{$si_data_ref}){
     ...
     $test = 1;
     my $page_name = $si_data_ref->{$data_type_comp}->{web_page_name};
     my $record_count = &commify($si_data_ref->{$data_type_comp}->{record_count});
  ...
  #Group Differential Splicing data
  $record = "<TD CLASS=\"Data2\">N/A</TD>";
  $test = 0;
  foreach my $data_type_comp (sort {$gsi_data_ref->{$a}->{line_order} <=> $gsi_data_ref->{$b}->{line_order}} keys %{$gsi_data_ref}){
     my $data_type = $gsi_data_ref->{$data_type_comp}->{data_type};
     ...
     $test = 1;
     my $page_name = $gsi_data_ref->{$data_type_comp}->{web_page_name};
     my $record_count = &commify($gsi_data_ref->{$data_type_comp}->{record_count});
     my $img_desc = "Alternative Expression report for $data_type";
     $record = "<TD CLASS=\"Data2\"><A HREF=\"$page_name\"><IMG SRC=\"$ae_icon\" CLASS=\"Pic_unbordered\" ALT=\"$img_desc\" TITLE=\"$img_desc\"></A><BR>($record_count)</TD>";
     $summary_content = "$summary_content"."$record";
  }
  unless ($test){
    foreach my $comparison_name (sort {$group_comp_list{$a}{line_order} <=> $group_comp_list{$b}{line_order}} keys %group_comp_list){
      $summary_content = "$summary_content"."<TD CLASS=\"Data2\">N/A</TD>";
    }
  }
