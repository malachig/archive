...
#Reverse junction probe intensities are also moved from their own line to an additional field in line for the corresponding 
#experimental probe
...
sub combineProbeData{
  ...
  foreach my $probe_id (keys %probe_data){
    ...
    #Probes with reverse-junction probe pairs.  Exon-Exon, Exon-Intron, Intron-Exon, Control-Negative
    if ($probe_type eq "Exon-Exon" || $probe_type eq "Exon-Intron" || $probe_type eq "Intron-Exon" || $probe_type eq "Control-Negative"){
      ...
      $probe_summary{$probe_id}{intensities_control} = $probe_data{$probe_pair_id}{intensities};

      $probe_count += 2;
      $record_count++;
  
    }elsif($probe_type eq "Exon"){
      #Remember that some probes (Exon probes) do not have these paired reverse-junction probes.
      $probe_summary{$probe_id}{gene_id} = $probes{$probe_id}{gene_id};
      $probe_summary{$probe_id}{target_id} = $probe_data{$probe_id}{target_id};
      $probe_summary{$probe_id}{probe_type} = $probe_type;
      $probe_summary{$probe_id}{sequence} = $probes{$probe_id}{sequence};
      ...
      #Create 'na' values for exon probes which do not have corresponding reverse junction probe pairs
      my $experiment_count = @experiment_ids;
      my @null_intensities;
      for (my $i = 0; $i <= $experiment_count-1; $i++){
      	push(@null_intensities, "na");
      }
      $probe_summary{$probe_id}{probe_pair_id} = $probes{$probe_id}{probe_pair_id};
      $probe_summary{$probe_id}{intensities_control} = \@null_intensities;
...
sub printSummaryDataFile{
  ...
  foreach my $experiment_id (@experiment_ids){
    my $probe_pair_name = "$experiment_id"."_revJunct";
    print OUTFILE "\t$experiment_id\t$probe_pair_name";
  }
  print OUTFILE "\n";  

  foreach my $probe_id (sort {$probe_summary{$b}->{target_id} cmp $probe_summary{$a}->{target_id}} keys %probe_summary){
    ...
    my @control_intensities = @{$probe_summary{$probe_id}{intensities_control}};
    foreach my $experimental_intensity (@experimental_intensities){
      my $control_intensity = shift(@control_intensities);
      print OUTFILE "\t$experimental_intensity\t$control_intensity";
    }
    print OUTFILE "\n";
