...
#    - For example:
#    - ~/NimbleGen_Data/MIPvs5FUR_04Dec2006/AuxillaryData/Processed_data/Quantiles_TmBGC/MTP_formatted/SigEvents_Log2Exp_FilteredOnExp_DE.txt
#    AND
#    - ~/NimbleGen_Data/MIPvs5FUR_04Dec2006/AuxillaryData/Processed_data/Quantiles_TmBGC/SI_values/MTP_formatted/SigEvents_Log2Exp_FilteredOnExp_SI.txt
...
#6.) Import probeset-level DE and SI datafiles and p-value files

#    - DE:
#    - ~/NimbleGen_Data/MIPvs5FUR_04Dec2006/AuxillaryData/Processed_data/Quantiles_TmBGC/Standard_probeset_data.txt  abs(mip_v_fur_DE_U)
#    - ~/NimbleGen_Data/MIPvs5FUR_04Dec2006/AuxillaryData/Processed_data/Quantiles_TmBGC/MTP_formatted/exon_log2_data_pvals.txt  (raw_ttest_pval)
#    - SI:
#    - ~/NimbleGen_Data/MIPvs5FUR_04Dec2006/AuxillaryData/Processed_data/Quantiles_TmBGC/SI_values/Standard_probeset_data.txt  abs(mip_v_fur_SI_U)
#    - ~/NimbleGen_Data/MIPvs5FUR_04Dec2006/AuxillaryData/Processed_data/Quantiles_TmBGC/SI_values/MTP_formatted/exon_log2_data_pvals.txt  (raw_ttest_pval)

#7.) For each gene make note of the following:
#    - For canonical probesets (exon, canonical junctions):
#      - Max abs(DE) value, Top P-value, Number of probesets with abs(DE) > 2-fold.
#      - Max abs(SI) value, Top P-value, Number of probesets with abs(SI) > 2-fold.
#    - For alternate probesets (skipping, exon-boundary)
#      - Max abs(DE) value, Top P-value, Number of probesets with abs(DE) > 2-fold.
#      - Max abs(SI) value, Top P-value, Number of probesets with abs(SI) > 2-fold.

#8.) For each gene, summarize the best MTP corrected P-value that passed filtering and was imported in (1).
#    - Note that many genes may not have any of these, set everyting to 'na'
#    - Report the following for each gene:
#      - DE values:
#        - # significant probesets, top adjusted p-value, # within ORF, # with 3' UTR, # within 5' UTR,
#        - # affecting TMDs, # affecting SPs, # affecting CCs, # affecting protein domains, non-redundant list of protein domains affected
#      - SI values:
#        - # significant probesets, top adjusted p-value, # within ORF, # with 3' UTR, # within 5' UTR,
#        - # affecting TMDs, # affecting SPs, # affecting CCs, # affecting protein domains, non-redundant list of protein domains affected
...
#1.) Import lists of significant DE and SI events
my %sig_probesets_list;
my %sig_genes_list;

my $DE_sigEventsFile = "/home/malachig/AlternativeSplicing/Array_analysis/NimbleGen_Data/MIPvs5FUR_04Dec2006/AuxillaryData/Processed_data/Quantiles_TmBGC/MTP_formatted/SigEvents_Log2Exp_FilteredOnExp_DE.txt";
my $SI_sigEventsFile = "/home/malachig/AlternativeSplicing/Array_analysis/NimbleGen_Data/MIPvs5FUR_04Dec2006/AuxillaryData/Processed_data/Quantiles_TmBGC/SI_values/MTP_formatted/SigEvents_Log2Exp_FilteredOnExp_SI.txt";

#Create a hash of these events, keyed on probeset ID
my $sig_DE_events_ref = &parseSig_DE_EventsFiles('-DE_file'=>$DE_sigEventsFile);
my $sig_SI_events_ref = &parseSig_SI_EventsFiles('-SI_file'=>$SI_sigEventsFile);
...
my @gene_ids = keys %sig_genes_list;
...
#4.) Print out an appended file. Same an input file except it has the info gathered in (2) and (3) added
my $de_out_file = "/home/malachig/AlternativeSplicing/Array_analysis/NimbleGen_Data/MIPvs5FUR_04Dec2006/AuxillaryData/Processed_data/Quantiles_TmBGC/MTP_formatted/SigEvents_Log2Exp_FilteredOnExp_DE_ANNOTATED.txt";
my $si_out_file = "/home/malachig/AlternativeSplicing/Array_analysis/NimbleGen_Data/MIPvs5FUR_04Dec2006/AuxillaryData/Processed_data/Quantiles_TmBGC/SI_values/MTP_formatted/SigEvents_Log2Exp_FilteredOnExp_SI_ANNOTATED.txt";

printOutputFiles('-de_file'=>$de_out_file, '-si_file'=>$si_out_file);
...
#1.) Import lists of significant DE and SI events
#########################################################################################################
sub parseSig_DE_EventsFiles{
  my %args = @_;
  my $de_file = $args{'-DE_file'};

  my %de_events;
  print BLUE, "\n\n1-A.) Processing input file: $de_file\n", RESET;

  #ProbeSet_ID, AlexaGene_ID, Probe_Type, Exons_Skipped, mip_v_fur_DE_U, adjp_BH, gene_mip_v_fur_DE_U
  ...
  while(<DE>){
    ...
    $sig_probesets_list{$probeset_id}{gene_id} = $line[1];
    $sig_probesets_list{$probeset_id}{tmd_count} = 0;
    $sig_probesets_list{$probeset_id}{sp_count} = 0;
    $sig_probesets_list{$probeset_id}{cc_count} = 0;
    $sig_probesets_list{$probeset_id}{domain_count} = 0;
    my @domain_names;
    $sig_probesets_list{$probeset_id}{domain_names} = \@domain_names;
    $sig_probesets_list{$probeset_id}{novel_known} = 'na';
    $sig_probesets_list{$probeset_id}{est_support} = 'na';

    $sig_genes_list{$line[1]}{tmp} = '';

    $de_events{$probeset_id}{gene_id} = $line[1];
    $de_events{$probeset_id}{probe_type} = $line[2];
    $de_events{$probeset_id}{exons_skipped} = $line[3];
    $de_events{$probeset_id}{de_value} = $line[4];
    $de_events{$probeset_id}{adjusted_pvalue} = $line[5];
    $de_events{$probeset_id}{gene_de_value} = $line[6];
    $de_events{$probeset_id}{line_count} = $line_count;
  }
  close (DE);

  my $de_events_count = keys %de_events;

  print BLUE, "\nImported $de_events_count significant DE events\n\n", RESET;

  return(\%de_events);
}


sub parseSig_SI_EventsFiles{
  my %args = @_;
  my $si_file = $args{'-SI_file'};

  my %si_events;
  print BLUE, "\n\n1-B.) Processing input file: $si_file\n", RESET;

  #ProbeSet_ID, AlexaGene_ID, Probe_Type, Exons_Skipped, mip_v_fur_SI_U, adjp_BH, gene_mip_v_fur_DE_U
  open (SI, "$si_file") || die "\nCould not open SI file: $si_file\n\n";
  ...
  while(<SI>){
    ...
    $si_events{$probeset_id}{gene_id} = $line[1];
    $si_events{$probeset_id}{probe_type} = $line[2];
    $si_events{$probeset_id}{exons_skipped} = $line[3];
    $si_events{$probeset_id}{si_value} = $line[4];
    $si_events{$probeset_id}{adjusted_pvalue} = $line[5];
    $si_events{$probeset_id}{gene_de_value} = $line[6];
    $si_events{$probeset_id}{line_count} = $line_count;
  }
  close (SI);

  my $si_events_count = keys %si_events;

  print BLUE, "\nImported $si_events_count significant SI events\n\n", RESET;
...
sub importProbeData{
  ...
  while (<PROBES>){
    ...
    unless ($sig_probesets_list{$probeset_id}){
      next();
    }
...
sub identifyEventPositions{
  ...
  foreach my $probeset_id (keys %sig_probesets_list){

    my $gene_id = $sig_probesets_list{$probeset_id}{gene_id};
    ...
    $sig_probesets_list{$probeset_id}{position} = 'na';
    if ($test_end < $grand_cds_start){
      $sig_probesets_list{$probeset_id}{position} = "5prime_UTR";
    }
    if ($test_start > $grand_cds_end){
      $sig_probesets_list{$probeset_id}{position} = "3prime_UTR";
    }
    if (($test_end >= $grand_cds_start && $test_end <= $grand_cds_end) || ($test_start >= $grand_cds_start && $test_start <= $grand_cds_end)){
      $sig_probesets_list{$probeset_id}{position} = "ORF";
    }
...
sub identifyOverlapingProteinFeatures{
  ...
  foreach my $probeset_id (keys %sig_probesets_list){
	...
	if ($overlap_found == 1){
	  if ($type eq "tmhmm"){
	    $sig_probesets_list{$probeset_id}{tmd_count}++;
	  }
	  if ($type eq "Signalp"){
	    $sig_probesets_list{$probeset_id}{sp_count}++;
	  }
	  if ($type eq "ncoils"){
	    $sig_probesets_list{$probeset_id}{cc_count}++;
	  }
	  if ($type eq "Pfam" || $type eq "pfscan" || $type eq "Prints" || $type eq "scanprosite"){
	    $sig_probesets_list{$probeset_id}{domain_count}++;
	    push(@{$sig_probesets_list{$probeset_id}{domain_names}}, $combo_name);
	  }
	}
      }
...
sub identifyNovelKnownProbesets{
  ...
  foreach my $probeset_id (keys %sig_probesets_list){
    ...
    $sig_probesets_list{$probeset_id}{novel_known} = "novel";
    $sig_probesets_list{$probeset_id}{est_support} = "NO";

    if ($enst_match_percent >= 95){
      $sig_probesets_list{$probeset_id}{novel_known} = "known";
    }
    if ($mrna_match_percent >= 95){
      $sig_probesets_list{$probeset_id}{novel_known} = "known";
    }
    if ($est_match_percent >= 95){
      $sig_probesets_list{$probeset_id}{est_support} = "YES";
    }
  }
...
sub printOutputFiles{
  my %args = @_;
  my $de_out_file = $args{'-de_file'};
  my $si_out_file = $args{'-si_file'};

  #DE OUT FILE
  open (DE_OUT, ">$de_out_file") || die "\nCould not open output file: $de_out_file\n\n";

  print DE_OUT "ProbeSet_ID\tAlexaGene_ID\tProbe_Type\tExons_Skipped\tmip_v_fur_DE_U\tadjp_BH\tgene_mip_v_fur_DE_U\tProbePosition\tTMD_count\tSignalPeptide_count\tCoiledCoil_count\tProteinDomainCount\tNovel_or_Known\tEST_Support\tProteinDomainNames\n";

  foreach my $probeset_id (sort {$sig_DE_events_ref->{$a}->{line_count} <=> $sig_DE_events_ref->{$b}->{line_count}} keys %{$sig_DE_events_ref}){
    print DE_OUT "$probeset_id\t$sig_DE_events_ref->{$probeset_id}{gene_id}\t$sig_DE_events_ref->{$probeset_id}{probe_type}\t$sig_DE_events_ref->{$probeset_id}{exons_skipped}\t$sig_DE_events_ref->{$probeset_id}{de_value}\t$sig_DE_events_ref->{$probeset_id}{adjusted_pvalue}\t$sig_DE_events_ref->{$probeset_id}{gene_de_value}\t$sig_probesets_list{$probeset_id}{position}\t$sig_probesets_list{$probeset_id}{tmd_count}\t$sig_probesets_list{$probeset_id}{sp_count}\t$sig_probesets_list{$probeset_id}{cc_count}\t$sig_probesets_list{$probeset_id}{domain_count}\t$sig_probesets_list{$probeset_id}{novel_known}\t$sig_probesets_list{$probeset_id}{est_support}\t@{$sig_probesets_list{$probeset_id}{domain_names}}\n";
  }
  close (DE_OUT);

  #SI OUT FILE
  open (SI_OUT, ">$si_out_file") || die "\nCould not open output file: $si_out_file\n\n";

  print SI_OUT "ProbeSet_ID\tAlexaGene_ID\tProbe_Type\tExons_Skipped\tmip_v_fur_SI_U\tadjp_BH\tgene_mip_v_fur_DE_U\tProbePosition\tTMD_count\tSignalPeptide_count\tCoiledCoil_count\tProteinDomainCount\tNovel_or_Known\tEST_Support\tProteinDomainNames\n";

  foreach my $probeset_id (sort {$sig_SI_events_ref->{$a}->{line_count} <=> $sig_SI_events_ref->{$b}->{line_count}} keys %{$sig_SI_events_ref}){
    print SI_OUT "$probeset_id\t$sig_SI_events_ref->{$probeset_id}{gene_id}\t$sig_SI_events_ref->{$probeset_id}{probe_type}\t$sig_SI_events_ref->{$probeset_id}{exons_skipped}\t$sig_SI_events_ref->{$probeset_id}{si_value}\t$sig_SI_events_ref->{$probeset_id}{adjusted_pvalue}\t$sig_SI_events_ref->{$probeset_id}{gene_de_value}\t$sig_probesets_list{$probeset_id}{position}\t$sig_probesets_list{$probeset_id}{tmd_count}\t$sig_probesets_list{$probeset_id}{sp_count}\t$sig_probesets_list{$probeset_id}{cc_count}\t$sig_probesets_list{$probeset_id}{domain_count}\t$sig_probesets_list{$probeset_id}{novel_known}\t$sig_probesets_list{$probeset_id}{est_support}\t@{$sig_probesets_list{$probeset_id}{domain_names}}\n";
  }
  close (SI_OUT);
