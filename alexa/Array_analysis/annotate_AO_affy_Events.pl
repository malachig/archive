...
#The purpose of this script is to summarize DE or SI events detected at the probeset level (exons or introns) for Affy data
#Specifically all the Affy probesets which correspond to genes targeted by the ALEXA_hs_35_35h design


#EVENT LEVEL SUMMARY
#1.) Import list of all DE probesets
#    - For example:
#    - ~NimbleGen_Data/MIPvs5FUR_04Dec2006/AuxillaryData/Processed_data/Quantiles_TmBGC/AO_probesets/MTP_formatted/SigEvents_Log2Exp_FilteredOnExp_DE.txt
...
my %de_probesets_list;
my %de_genes_list;

my $de_EventsFile = "/home/malachig/AlternativeSplicing/Array_analysis/NimbleGen_Data/MIPvs5FUR_04Dec2006/AuxillaryData/Processed_data/Quantiles_TmBGC/AO_probesets/MTP_formatted/SigEvents_Log2Exp_FilteredOnExp_DE.txt";

#Create a hash of these events, keyed on probeset ID
my $de_events_ref = &parse_EventsFile('-de_file'=>$de_EventsFile);

#print Dumper %de_probesets_list;
#exit();

#print Dumper $de_events_ref;
#exit();


#2.) For each of these events get info for the corresponding probesets.
my $probe_file1 = "/home/malachig/AlternativeSplicing/perl_bin/Affy_analysis/mapfiles/hg17/COMPLETE_probesets_mapped_to_ensembl.txt";
my $probe_file2 = "/home/malachig/AlternativeSplicing/perl_bin/Affy_analysis/mapfiles/hg17/CORE_probesets_mapped_to_ensembl.txt";
my %probesets;
&importProbeData('-input_file1'=>$probe_file1, '-input_file2'=>$probe_file2);

#print Dumper %probesets;
#exit();

#3.) Identify features for each probeset.
my $alexa_dbh = &connectDB('-database'=>'ALEXA_hs_35_35h', '-server'=>'jango.bcgsc.ca', '-user'=>'malachig', '-password'=>'gEEnom$');
my @gene_ids = keys %de_genes_list;
...
#4.) Print out an appended file. Same an input file except it has the info gathered in (2) and (3) added
my $out_file = "/home/malachig/AlternativeSplicing/Array_analysis/NimbleGen_Data/MIPvs5FUR_04Dec2006/AuxillaryData/Processed_data/Quantiles_TmBGC/AO_probesets/MTP_formatted/SigEvents_Log2Exp_FilteredOnExp_DE_ANNOTATED.txt";
...
sub parse_EventsFile{
  ...
  while(<DE>){
    ...
    $de_probesets_list{$probeset_id}{gene_id} = $line[1];
    $de_probesets_list{$probeset_id}{tmd_count} = 0;
    $de_probesets_list{$probeset_id}{sp_count} = 0;
    $de_probesets_list{$probeset_id}{cc_count} = 0;
    $de_probesets_list{$probeset_id}{domain_count} = 0;
    my @domain_names;
    $de_probesets_list{$probeset_id}{domain_names} = \@domain_names;
    $de_probesets_list{$probeset_id}{novel_known} = 'na';
    $de_probesets_list{$probeset_id}{est_support} = 'na';

    unless ($line[1] eq "0"){
      $de_genes_list{$line[1]}{tmp} = '';
    }
  ...
  print BLUE, "\nImported $de_events_count events\n\n", RESET;
...
sub importProbeData{
  my %args = @_;
  my $probe_file1 = $args{'-input_file1'};
  my $probe_file2 = $args{'-input_file2'};

  #Open the probe file and read the neccessary probe data into a hash keyed on probe ID
  open (PROBES1, "$probe_file1") || die "\nCould not open input probe file1: $probe_file1";
  ...
  while (<PROBES1>){
    ...
    #ProbeSet_ID
    my $probeset_id = $probe_line[$columns{probe_set_id}{column_pos}];

    #Unless this probeset is in the list of significant events, skip it
    unless ($de_probesets_list{$probeset_id}){
      next();
    }

    my $gene_id = $probe_line[$columns{alexa_gene_id}{column_pos}];
    my $probe_length = 25;
    my $probe_type = $de_events_ref->{$probeset_id}->{probe_type};

    my $unit1_start = $probe_line[$columns{start}{column_pos}];
    my $unit1_end = $probe_line[$columns{stop}{column_pos}];
    my $unit2_start = 'na';
    my $unit2_end = 'na';
    my $enst_target_hit_length = 'na';
    my $mrna_target_hit_length = 'na';
    my $est_target_hit_length = 'na';
  ...
  #Open the probe file and read the neccessary probe data into a hash keyed on probe ID
  open (PROBES2, "$probe_file2") || die "\nCould not open input probe file2: $probe_file2";
  ...
  while (<PROBES2>){
    ...
    #If this probeset was already found above, skip it
...
sub identifyEventPositions{
  ...
  foreach my $probeset_id (keys %de_probesets_list){

    unless ($probesets{$probeset_id}){
      print RED, "\nProbeset_ID = $probeset_id\n", RESET;
      next();
    }

    my $gene_id = $de_probesets_list{$probeset_id}{gene_id};
    my $probe_type = $probesets{$probeset_id}{probe_type};

    $de_probesets_list{$probeset_id}{position} = 'na';
    ...
    my $coords_ref = &convertGeneCoordinates ('-dbh'=>$alexa_dbh, '-gene_id'=>$gene_id, '-start_pos'=>$grand_cds_start, '-end_pos'=>$grand_cds_end);
    $grand_cds_start = $coords_ref->{$gene_id}->{chr_start};
    $grand_cds_end = $coords_ref->{$gene_id}->{chr_end};
    ...
    if ($test_end < $grand_cds_start){
      $de_probesets_list{$probeset_id}{position} = "5prime_UTR";
    }
    if ($test_start > $grand_cds_end){
      $de_probesets_list{$probeset_id}{position} = "3prime_UTR";
    }
    if (($test_end >= $grand_cds_start && $test_end <= $grand_cds_end) || ($test_start >= $grand_cds_start && $test_start <= $grand_cds_end)){
      $de_probesets_list{$probeset_id}{position} = "ORF";
    }
  }
  return();
}
...
sub identifyOverlapingProteinFeatures{
  ...
  foreach my $probeset_id (keys %de_probesets_list){
	...
	foreach my $pf_start (@start_coords){
	  ...
	  my $coords_ref = &convertGeneCoordinates ('-dbh'=>$alexa_dbh, '-gene_id'=>$gene_id, '-start_pos'=>$pf_start, '-end_pos'=>$pf_end);
	  $pf_start = $coords_ref->{$gene_id}->{chr_start};
	  $pf_end = $coords_ref->{$gene_id}->{chr_end};
	...
	if ($overlap_found == 1){
	  if ($type eq "tmhmm"){
	    $de_probesets_list{$probeset_id}{tmd_count}++;
	  }
	  if ($type eq "Signalp"){
	    $de_probesets_list{$probeset_id}{sp_count}++;
	  }
	  if ($type eq "ncoils"){
	    $de_probesets_list{$probeset_id}{cc_count}++;
	  }
	  if ($type eq "Pfam" || $type eq "pfscan" || $type eq "Prints" || $type eq "scanprosite"){
	    $de_probesets_list{$probeset_id}{domain_count}++;
	    push(@{$de_probesets_list{$probeset_id}{domain_names}}, $combo_name);
	  }
	}
      }
    }
...
sub identifyNovelKnownProbesets{
  ...
  foreach my $probeset_id (keys %de_probesets_list){
    ...
    $de_probesets_list{$probeset_id}{novel_known} = "novel";
    $de_probesets_list{$probeset_id}{est_support} = "NO";

    if ($enst_match_percent >= 95){
      $de_probesets_list{$probeset_id}{novel_known} = "known";
    }
    if ($mrna_match_percent >= 95){
      $de_probesets_list{$probeset_id}{novel_known} = "known";
    }
    if ($est_match_percent >= 95){
      $de_probesets_list{$probeset_id}{est_support} = "YES";
    }
  }
  return();
}
...
sub printOutputFiles{
  ...
  foreach my $probeset_id (sort {$de_events_ref->{$a}->{line_count} <=> $de_events_ref->{$b}->{line_count}} keys %{$de_events_ref}){
    print OUT "$probeset_id\t$de_events_ref->{$probeset_id}->{gene_id}\t$de_events_ref->{$probeset_id}->{probe_type}\t$de_events_ref->{$probeset_id}->{exons_skipped}\t$de_probesets_list{$probeset_id}{position}\t$de_probesets_list{$probeset_id}{tmd_count}\t$de_probesets_list{$probeset_id}{sp_count}\t$de_probesets_list{$probeset_id}{cc_count}\t$de_probesets_list{$probeset_id}{domain_count}\t$de_probesets_list{$probeset_id}{novel_known}\t$de_probesets_list{$probeset_id}{est_support}\t@{$de_probesets_list{$probeset_id}{domain_names}}\n";
  }
  close (OUT);
