...
@EXPORT_OK = qw(&checkMaxPacketSize &getAllGenes &getGeneIds &getGeneInfo &getTranscripts &getProbeInfo &getGeneProbes &getNcProbes &getExons
		&getMaskedExons &getExonContent &getIntronContent &getMaskedGene &getGeneTerms &junctionProbeCombinations &displayGeneStats
...
sub getTranscripts{
  ...
  foreach my $gene (@gene_ids){
	...
	$| = 0;
      ...
      my $sql_transcript = "SELECT fk_Gene__id, id, ensembl_t_id, start, end FROM Transcript WHERE fk_Gene__id IN ($id_string)";
      my $sth_transcript = $dbh->prepare("$sql_transcript");
      $sth_transcript->execute();

      while (my ($fk_gene_id,$transcript_id,$ensembl_t_id,$transcript_start,$transcript_end) = $sth_transcript->fetchrow_array()){
