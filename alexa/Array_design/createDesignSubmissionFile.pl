...
GetOptions ('database=s'=>\$database, 'server=s'=>\$server, 'user=s'=>\$user, 'password=s'=>\$password,
	    ...
	    'gene_type=s'=>\$gene_type, 'known_genes_only=s'=>\$known_genes_only, 'gene_list=s'=>\$gene_list,
...
print GREEN, "\n\t\tTo target all genes use: --gene_type=All (other valid options are 'Non-pseudo' and 'protein_coding')", RESET;
print GREEN, "\n\t\tTo also limit to only 'known' genes use: --known_genes_only=yes", RESET;
...
unless (($gene_type =~ /^All$|^protein_coding$|^Non-pseudo$/) || $gene_list){
  print RED, "\nYou must specify an input gene list with --gene_list or use the --gene_type option\n\n", RESET;
  exit();
}
...
#    - Check all gene IDs to make sure they exist in the specified database
my @target_genes;
if ($gene_type =~ /^All$|^protein_coding$|^Non-pseudo$/){
  #Note that even if you specify All genes here, they may not have all had probes selected (most likely pseudogenes have been excluded at the least)

  if ($known_genes_only =~ /^yes/i){
    @target_genes = @{&getAllGenes ('-dbh'=>$alexa_dbh, '-gene_type'=>$gene_type, '-evidence'=>"Known Gene")};
  }else{
    @target_genes = @{&getAllGenes ('-dbh'=>$alexa_dbh, '-gene_type'=>$gene_type)};
  }
...
my $log_file = "$out_dir"."createDesignSubmissionFile_LOG.txt";
...
open (LOG, ">$log_file") || die "\nCould not open log file for writing: $log_file\n\n";

print LOG "Specified the following parameters on input:\n\tdatabase = $database\n\tserver = $server\n\tuser = $user\n\tpassword = pwd\n\tgene_type = $gene_type\n\tgene_list = $gene_list\n\tknown_genes_only = $known_genes_only\n\ttarget_tm = $target_tm\n\ttarget_length = $target_length\tmax_length_variance = $max_length_variance\n\tprobeset_size = $probeset_size\n\tfiltered_probes = $filtered_probes\n\texon_skip_limit = $exon_skip_limit\n\tknown_events_only = $known_events_only\n\tarray_capacity = $array_capacity\n\tmin_gene_probe_coverage = $min_gene_probe_coverage\n\tmax_exons = $max_exons\n\tmin_nc_probes = $min_nc_probes\n\tout_dir = $out_dir";
...
print LOG "\n\n$too_many_exons_count genes had too many exons and were skipped";
print LOG "\n$insufficient_coverage_count genes had insufficient gene probe coverage and were skipped";
print LOG "\n$targeted_count genes were approved and successfully targeted";
print LOG "\n$no_space_count genes were approved but could not be targeted because the specified array capacity ran out\n\n";
...
sub importGeneList{
  ...
  print LOG "\n\tFound $input_gene_count genes in put gene list and $valid_gene_count of these were found in the ALEXA database";
  ...
  unless ($input_gene_count == $valid_gene_count){
    ...
    print LOG "\n\nThe following genes in the input list were not found in the ALEXA database specified";
    ...
    print LOG "\n\nThese IDs are either invalid or belong to a different database version or species than the one you specified";
    print LOG "\nFix this issue and try again\n\n";
    exit();
  }
...
sub getGeneData{
  ...
  my $multiple_exon_gene_count = scalar(@multi_exon_genes);

  if ($multiple_exon_gene_count > 0){
...
sub selectNegativeControlProbes{
  ...
  if ($filtered =~ /^yes$/i){
    ...
    print LOG "\n\tGetting all Unfiltered Negative Control probes from ALEXA -may take a while";
    print LOG "\n\tNOTE: Using unfiltered probes is not recommended ...";
