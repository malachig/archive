...
#Basically this is just a wrapper for get_SimFold-PairFoldScores which simply uses the SimFold/PairFold binaries of MultiRNAFold
...
GetOptions ('master_probe_file=s'=>\$master_probe_file, 'expected_probe_count=i'=>\$expected_probe_count, 'pairfold_dir=s'=>\$pairfold_dir,
	    'folding_bin=s'=>\$folding_bin, 'temp_dir=s'=>\$temp_dir, 'batch_file=s'=>\$batch_file, 'repair=s'=>\$repair, 'staging_dir=s'=>\$staging_dir,
	    'logfile=s'=>\$logfile);
...
print GREEN, "\n\tSpecify the full path to the directory containing pairfold executables using:  --pairfold_dir (eg. /alexa/BioSw/MultiRNAFold-1.8/)", RESET;
...
print GREEN, "\n\tIf you want to copy the pairfold program over to a staging directory on each node of the cluster to improve performance, use:  --staging_dir=/tmp", RESET;
print GREEN, "\n\t\tOtherwise use: --staging_dir=no", RESET;
print GREEN, "\n\tAlso specify the name of a log file to be used to store a record of this script: --logfile", RESET;
print GREEN, "\n\nExample: createPairFoldBatch.pl  --master_probe_file=/home/user/alexa/ALEXA_version/unfiltered_probes/exonJunctionProbes.txt  --expected_probe_count=1234  --pairfold_dir=~/MultiRNAFold-1.8/  --folding_bin=~/alexa/Array_design/get_SimFold-PairFoldScores.pl  --temp_dir=/home/user/alexa/ALEXA_version/fold/  --batch_file=/home/user/alexa/ALEXA_version/batch_scripts/pairfold_ExonJunction.sh  --repair=no  --staging_dir=/tmp/  --logfile=/home/user/alexa/ALEXA_version/logs/createPairFoldBatch_ExonJunction_LOG.txt\n\n", RESET;

unless ($master_probe_file && $expected_probe_count && $pairfold_dir && $folding_bin && $temp_dir && $batch_file && $staging_dir && $logfile && $repair){
...
#Deal with the --staging_dir option:
# - create a command to copy the pairfold program directory to the specified local directory on each node
# - also create to remove this copy at the end of the job
my $copy_command = '';
my $remove_command1 = '';
my $remove_command2 = '';

if($staging_dir eq "no"){
  print YELLOW, "\nNo staging directory specified.  Staging is strongly recommended for cluster jobs (unless the program already exists on all nodes ...)\n\n", RESET;
  print LOG "\nNo staging directory specified.  Staging is strongly recommended for cluster jobs (unless the program already exists on all nodes ...)\n\n";
}else{
  print BLUE, "\nCreating commands to allow staging of pairfold files to local directories of each node of the cluster\n\n", RESET;
  print LOG "\nCreating commands to allow staging of pairfold files to local directories of each node of the cluster\n\n";

  unless ($staging_dir =~ /.*\/$/){
    $staging_dir = "$staging_dir"."/";
  }
  $copy_command = "cp -r $pairfold_dir $staging_dir; ";

  #Get the name of the pairfold_dir
  my $pairfold_name;
  #$_=~/\/.+?\/$/
  if ($pairfold_dir =~ /\/([a-zA-z0-9\.\-_]+)\/$/){
    $pairfold_name = $1;
  }else{
    print RED, "\nCould not get pairfold dir name from path: $pairfold_dir\n\n";
    exit();
  }

  my $local_pairfold_dir = "$staging_dir"."$pairfold_name"."/";
  $remove_command1 = "rm -fr $local_pairfold_dir; ";
  $remove_command2 = "; rm -fr $local_pairfold_dir";

  #Update the pairfold path so that the local one is used on each node of the cluster
  $pairfold_dir = $local_pairfold_dir;
}
...
print BATCHFILE "$remove_command1$copy_command$folding_bin --probe_column $probe_sequence_column --infile $probe_file --outfile $out_file --pairfold_dir $pairfold_dir$remove_command2\n";
$probe_files_printed++;
