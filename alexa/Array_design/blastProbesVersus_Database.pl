...
print GREEN, "\n\tSpecify the type of blast database being used using: --db_type ('EST', 'mRNA', 'genomic', 'enst', or 'probe')", RESET;
...
if ($db_type eq "mRNA" || $db_type eq "EST" || $db_type eq "genomic" || $db_type eq "enst" || $db_type eq "probe"){
  ...
  print RED, "\nMust specify a valid database type ('mRNA', 'EST', 'genomic', 'enst', or 'probe')\n\n", RESET;
  close (LOG);
  exit();
}
...
if ($db_type eq "genomic"){
  unless ($probe_parse_bin =~ /Negative\.pl/){
    print RED, "\nThe 'genomic' option should only be used in conjuction with testProbeSpecificity_Negative.pl!!\n\n", RESET;
    close (LOG);
    exit();
  }
}
